...
nodewith does not actively find a node, it just describes what to look for.
...
Next, there is the [uiauto.Context] created with [uiauto.New(tconn)].
The ui context is focused on creating a flake resistant way to interact with a11y tree.
In order to reduce flakes, ui actions internally do polling and checks for stable locations.
...
[uiauto.New(tconn)]: https://pkg.go.dev/chromium.googlesource.com/chromiumos/platform/tast-tests.git/src/chromiumos/tast/local/chrome/uiauto#New
...
if err := uiauto.Run(ctx,
  ui.RightClick(node1)(ctx),
  ui.LeftClick(node2)(ctx),
  ui.WaitUntilExists(node3)(ctx),
); err != nil {
...
race condition where the menu may not be fully ready to click by
...
work. The issue is that we do not always have a indicator for when the menu
button is ready to be click.

## More Basic Interactions

Now that the wallpaper picker is open, let's set the background to a solid color.
This is basically the same as above. We left click for the node corresponding to
the 'Solid colors' tab in `ui_tree.txt`:
```
