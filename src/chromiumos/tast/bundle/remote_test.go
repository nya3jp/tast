...
func TestRemoteMissingTarget(t *gotesting.T) {
	...
	defer restore()
	testing.AddRunnableTest(&testing.RunnableTest{Name: "pkg.Test", Func: func(context.Context, *testing.State) {}})
...
func TestRemoteDUT(t *gotesting.T) {
	...
	restore := testing.SetGlobalRegistryForTesting(testing.NewRegistry())
	defer restore()
	testing.AddRunnableTest(&testing.RunnableTest{Name: "pkg.Test", Func: func(ctx context.Context, s *testing.State) {
		dt := s.DUT()
...
func TestRemoteReconnectBetweenTests(t *gotesting.T) {
	...
	restore := testing.SetGlobalRegistryForTesting(testing.NewRegistry())
	defer restore()
	testing.AddRunnableTest(&testing.RunnableTest{Name: "pkg.Test1", Func: makeFunc(&conn1)})
	testing.AddRunnableTest(&testing.RunnableTest{Name: "pkg.Test2", Func: makeFunc(&conn2)})
