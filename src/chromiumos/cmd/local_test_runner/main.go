...
func main() {
	args := runner.Args{
		...
		SoftwareFeatureDefinitions: map[string]string{
			...
			// Grunt cannot run 720p due to performance issue, we should remove grunt after hardware encoding supported.
			"camera_720p":       "!snow && !skate && !spring && !grunt", // daisy variants' cameras don't support 1280x720
