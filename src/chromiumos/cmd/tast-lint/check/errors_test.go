...
func verifyIssues(t *testing.T, fs *token.FileSet, issues []*Issue, expects []string) {
	if len(issues) != len(expects) {
		t.Errorf("Unexpected number of issues: expected %d, got %d", len(expects), len(issues))
		return
	}
	...
	for i, issue := range issues {
		...
		expect := expects[i]
		if msg != expect {
			t.Errorf("Issue %d mismatch: expected %q, got %q", i, expect, msg)
		}
	}
}
...
`
	expects := []string{
		"testfile.go:10:2: chromiumos/tast/errors.Errorf should be used instead of fmt.Errorf",
	}
