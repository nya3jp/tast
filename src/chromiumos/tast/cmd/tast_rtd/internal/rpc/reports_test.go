...
func TestReportsServer_LogStream(t *testing.T) {
	...
	srv, err := NewReportsServer(0, psClient, testsToRequests, logSinkName)
	if err != nil {
	...
	if s := string(psServer.ReceivedLog(logSinkName, requestName1)); !strings.Contains(s, testLog1a+testLog1b) {
		t.Errorf("Log sent to test #1 was not forwarded. Log#1=%q", s)
	}
	if s := string(psServer.ReceivedLog(logSinkName, requestName1)); strings.Contains(s, testLog2a) {
		t.Errorf("Log sent to #2 appeared in #1. Log#1=%q", s)
	}
	if s := string(psServer.ReceivedLog(logSinkName, requestName2)); strings.Contains(s, testLog1a) {
...
func TestReportsServer_ReportResult(t *testing.T) {
	...
	reportsServer, err := NewReportsServer(0, psClient, testsToRequests, "log_sink_name")
	if err != nil {
		t.Fatalf("Failed to start Reports server: %v", err)
	}
