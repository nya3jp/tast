...
func TestLog(t *gotesting.T) {
	or := newOutputReader()
	s := newState(&RunnableTest{Timeout: time.Minute}, or.ch, &TestConfig{})
	s.Log("msg ", 1)
	s.Logf("msg %d", 2)
	close(or.ch)
	out := or.read()
...
func TestDataFileServer(t *gotesting.T) {
	...
	test := RunnableTest{Data: []string{file1}}
	or := newOutputReader()
...
func TestVars(t *gotesting.T) {
	...
	test := &RunnableTest{Vars: []string{validName, unsetName}}
...
func TestMeta(t *gotesting.T) {
	...
	getMeta := func(test *RunnableTest, cfg *TestConfig) (*State, *Meta) {
		or := newOutputReader()
		s := newState(test, or.ch, cfg)
	...
	if s, m := getMeta(&RunnableTest{Name: metaTest}, &TestConfig{RemoteData: &RemoteData{Meta: &meta}}); s.HasError() {
	...
	if s, m := getMeta(&RunnableTest{Name: nonMetaTest}, &TestConfig{RemoteData: &RemoteData{Meta: &meta}}); !s.HasError() {
	...
	if s, m := getMeta(&RunnableTest{Name: metaTest}, &TestConfig{}); !s.HasError() {
...
func TestRPCHint(t *gotesting.T) {
	...
	getHint := func(test *RunnableTest, cfg *TestConfig) (*State, *RPCHint) {
		or := newOutputReader()
		s := newState(test, or.ch, cfg)
	...
	if s, h := getHint(&RunnableTest{Name: remoteTest}, &TestConfig{RemoteData: &RemoteData{RPCHint: &hint}}); s.HasError() {
	...
	if s, h := getHint(&RunnableTest{Name: localTest}, &TestConfig{}); !s.HasError() {
...
func TestDUT(t *gotesting.T) {
	callDUT := func(test *RunnableTest, cfg *TestConfig) *State {
		or := newOutputReader()
		s := newState(test, or.ch, cfg)
	...
	if s := callDUT(&RunnableTest{Name: remoteTest}, &TestConfig{RemoteData: &RemoteData{}}); s.HasError() {
		t.Errorf("DUT() reported error for %v", remoteTest)
	}

	// Local tests shouldn't have access to DUT.
	if s := callDUT(&RunnableTest{Name: localTest}, &TestConfig{}); !s.HasError() {
