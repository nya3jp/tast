...
const (
	...
	// CompressedLogExt1 is the rightmost extension on the compressed log files written by crash_reporter. It
	// is the actual extension on the file.
	CompressedLogExt1 = ".gz"
	// CompressedLogExt2 is the second-from-the-right extension on the compressed log files written by crash_reporter.
	// It is the extension on the file after removing CompressedLogExt1.
	...
	for _, dir := range dirs {
		...
		for _, fn := range files {
			...
			if ext == CoreExt || ext == MinidumpExt || ext == LogExt || ext == MetadataExt || ext == GPUStateExt {
				...
				// Look for files that end in ".txt.gz". filepath doesn't give us a way
				// to look for extensions before the rightmost one, so we have to trim
				// the rightmost one to get at the next extension.
