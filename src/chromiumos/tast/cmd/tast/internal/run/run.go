...
func Run(ctx context.Context, cfg *Config) (status Status, results []*EntityResult) {
	...
	// TODO: do this before any downloading using devserver from DUT happens
	f, err := hst.NewRemoteForwarder(cfg.tlwServer, func(e error) {
		cfg.Logger.Logf("remote forwarder error: %s", e)
	})
	if err != nil {
		return errorStatusf(cfg, subcommands.ExitFailure, "Failed to reverse-forward a port: %v", err), nil
	}
