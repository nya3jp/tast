...
func startEphemeralDevserverAtLocalHost(ctx context.Context, cfg *Config, state *State) (*ephemeralDevserver, error) {
...
func runRemoteTests(ctx context.Context, cfg *Config, state *State) ([]*EntityResult, error) {
	...
	if cfg.tlwServer == "" && cfg.useEphemeralDevserver && len(state.remoteDevservers) == 0 {
		es, err := startEphemeralDevserverAtLocalHost(ctx, cfg, state)
		if err != nil {
