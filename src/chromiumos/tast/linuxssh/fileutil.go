...
func PutFiles(ctx context.Context, conn host.Conn, files map[string]string, symlinkPolicy SymlinkPolicy) (bytes int64, err error) {
	...
	cf, err := findChangedFiles(ctx, conn, af)
	if err != nil {
		return 0, err
	}
	if len(cf) == 0 {
	...
	rcmd := conn.Command("tar", "-x", "--gzip", "--no-same-owner", "--recursive-unlink", "-C", "/")
...
func findChangedFiles(ctx context.Context, conn host.Conn, files map[string]string) (map[string]string, error) {
	if len(files) == 0 {
		return nil, nil
	}
...
func getRemoteSHA1s(ctx context.Context, conn host.Conn, paths []string) (map[string]string, error) {
...
func DeleteTree(ctx context.Context, conn host.Conn, baseDir string, files []string) error {
	var cfs []string
	for _, f := range files {
	...
	cmd := conn.Command("rm", append([]string{"-rf", "--"}, cfs...)...)
	cmd.Dir = baseDir
