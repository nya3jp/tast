...
permanently bloating the test repository. Put _external link files_ named
`<original-name>.external` in `data/` subdirectory whose content is JSON in the
[external link format]:

```
{
  "url": "gs://chromiumos-test-assets-public/tast/<bundle-name>/<package-name>/<name>_<date>.<ext>",
  "size": <file-size>,
  "sha256sum": "<SHA256-hash>"
}
```

External data files are not installed to test images but are downloaded at
run time by `local_test_runner` on DUT.
