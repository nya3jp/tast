...
func buildFixtPlan(tests []*testing.TestInstance, pcfg *Config) (*fixtPlan, error) {
	...
	for _, t := range tests {
		testMap[t.Fixt] = append(testMap[t.Fixt], t)
	}
...
func (p *fixtPlan) run(ctx context.Context, st *fixtureStack, out OutputStream, dl *downloader) error {
	...
	log := func(msg string) {
		ti := &testing.TestInfo{Name: p.fixt.Name} // pseudo test for the fixture
		out.TestLog(ti, msg)
	}
	fixtCtx, cancel := context.WithCancel(testing.NewContext(context.Background(), tc, log))
	defer cancel()
...
func buildStages(t *testing.TestInstance, tout testing.OutputStream, pcfg *Config, precfg *preConfig, tcfg *testing.TestConfig) []stage {
	...
	root := testing.NewTestRoot(t, tout, tcfg)
