...
func ForbiddenCalls(fs *token.FileSet, f *ast.File, fix bool) []*Issue {
	...
	hasErrors := hasErrorsIdent(f)
	var canGoimports bool
	if src, err := bytesOfFile(f); err != nil {
		canGoimports = false
	} else {
		canGoimports = goimportApplicable(src)
	}
	...
	astutil.Apply(f, func(c *astutil.Cursor) bool {
		...
		case "fmt.Errorf":
			...
			fixable := !hasErrors && canGoimports
			if !fix {
				issues = append(issues, &Issue{
					Pos:     fs.Position(x.Pos()),
	...
	if errorsAdded {
		tastErrorsNode := &ast.ImportSpec{
			Path: &ast.BasicLit{
				Kind:  token.STRING,
				Value: strconv.Quote("chromiumos/tast/errors"),
			},
		}
