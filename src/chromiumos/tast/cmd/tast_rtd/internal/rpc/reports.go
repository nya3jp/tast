...
type ReportsServer struct {
	...
	// A template for formatting the progress sink name.
	// Must be a valid format string recognized by fmt.Sprintf containing "%s" as the sole verb there.
	// The "%s" is replaced with an individual test name.
	reportNameTmpl string
...
func (s *ReportsServer) LogStream(stream protocol.Reports_LogStreamServer) error {
	for {
		...
		req := rtd.ReportLogRequest{
			Name:    fmt.Sprintf(s.reportNameTmpl, in.Test),
			Request: request,
			Data:    in.Data,
		}
...
func NewReportsServer(port int, psClient rtd.ProgressSinkClient, testsToRequests map[string]string, reportNameTmpl string) (*ReportsServer, error) {
	...
	s := ReportsServer{
		...
		reportNameTmpl:   reportNameTmpl,
	}
	stream, err := psClient.ReportLog(context.Background())
	if err != nil {
		return nil, err
	}
	s.reportLogStream = &stream
