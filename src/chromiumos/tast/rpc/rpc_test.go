...
func TestRPCSuccess(t *gotesting.T) {
	ctx := testing.WithEntityContext(context.Background(), &testing.EntityContext{})
	called := false
	...
	callCtx := testing.WithEntityContext(ctx, &testing.EntityContext{
...
func TestRPCNoEntityContext(t *gotesting.T) {
	ctx := testing.WithEntityContext(context.Background(), &testing.EntityContext{})
	called := false
	pp := newPingPair(ctx, t, func(context.Context, *testing.ServiceState) error {
		called = true
	...
	if _, err := pp.Client.Ping(context.Background(), &PingRequest{}); err == nil {
		t.Error("Ping unexpectedly succeeded for a context missing EntityContext")
	}
	if called {
...
func TestRPCForwardEntityContext(t *gotesting.T) {
	...
	if _, err := pp.Client.Ping(ctx, &PingRequest{}); err == nil {
		t.Error("Ping unexpectedly succeeded for a context without EntityContext")
	}
...
func TestRPCForwardLogs(t *gotesting.T) {
	...
	ctx = testing.WithEntityContext(ctx, &testing.EntityContext{})
