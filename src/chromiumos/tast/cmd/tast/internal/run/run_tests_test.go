...
func TestFindPatternsForShard(t *gotesting.T) {
	...
	for shardIndex := 0; shardIndex < td.cfg.totalShards; shardIndex++ {
		td.cfg.shardIndex = shardIndex
		testNames, testsNotInShard, testsToSkip, err := findTestsForShard(context.Background(), &td.cfg)
		if err != nil {
			t.Fatal("Failed to find patterns for shard: ", err)
		}
		if len(testNames)+len(testsNotInShard)+len(testsToSkip) != len(tests) {
			t.Fatalf("The sum of numbers of tests in the shard (%v), outside the shard (%v) and skipped tests(%v) does not match the number of tests (%v)",
				len(testNames), len(testsNotInShard), len(testsToSkip), len(tests))
		}
