...
type stubRunWrapper struct {
	...
	runState, writeState *run.State                   // state passed to run and writeResults
...
func (w *stubRunWrapper) run(ctx context.Context, cfg *run.Config, state *run.State) (run.Status, []*jsonprotocol.EntityResult) {
	w.runCtx, w.runCfg, w.runState = ctx, cfg, state
	return w.runStatus, w.runRes
}

func (w *stubRunWrapper) writeResults(ctx context.Context, cfg *run.Config, state *run.State, results []*jsonprotocol.EntityResult, complete bool) error {
