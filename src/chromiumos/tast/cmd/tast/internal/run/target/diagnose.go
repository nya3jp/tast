...
// It returns a non-empty string if it finds any possible network problems.
func diagnoseNetwork(ctx context.Context, target string) string {
	opts := &ssh.Options{}
	...
	if err := tryResolveDNS(ctx, host); err != nil {
		...
		return fmt.Sprintf("DNS resolution failed: %s", msg)
	}
	...
	if err := tryPing(ctx, host); err != nil {
		...
		return fmt.Sprintf("ping failed: %s", msg)
	}
	...
	if err := tryRawConnect(ctx, host, port); err != nil {
		...
		return fmt.Sprintf("connect failed: %s", msg)
	}
