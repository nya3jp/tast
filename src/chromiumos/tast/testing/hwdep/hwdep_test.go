...
func verifyCondition(t *testing.T, c Condition, dc *device.Config, dut *configpb.HardwareFeatures, expectSatisfied bool) {
	t.Helper()

	err := c.Satisfied(&dep.HardwareFeatures{DC: dc, Features: dut})
	if expectSatisfied {
		if err != nil {
