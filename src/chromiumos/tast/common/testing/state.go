...
// It is intended to safe when called concurrently by multiple goroutines
// while a test is running.
type State struct {
	ch      chan Output        // channel to which logging messages and errors are written
	w       io.Writer          // used to write test logging messages
	errors  []Error            // errors encountered while running the test
	...
	mutex   sync.Mutex         // protects w and errors
}
...
// Failed returns true if the test failed and false if it passed.
...
// Errors returns all errors encountered while running the test.
...
// addError adds e to s.
func (s *State) addError(e *Error) {
	s.mutex.Lock()
