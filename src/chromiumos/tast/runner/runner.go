...
func runTestsAndReport(args *Args, stdout io.Writer) {
	...
	if len(tests) == 0 {
		...
		if !args.report && created {
			defer os.RemoveAll(bundleArgs.RunTestsArgs.OutDir)
		}
		...
		processExternalDataLinks(ctx, args.RunTestsArgs.DataDir, tests, cl, lf)
	...
	mw.WriteMessage(&control.RunEnd{Time: time.Now(), OutDir: bundleArgs.RunTestsArgs.OutDir})
}
...
// setUpBaseOutDir creates and assigns a temporary directory if args.RunTestsArgs.OutDir is empty.
// It also ensures that the dir is accessible to all users.
func setUpBaseOutDir(args *bundle.Args) (created bool, err error) {
	if args.RunTestsArgs.OutDir == "" {
		if args.RunTestsArgs.OutDir, err = ioutil.TempDir("", "tast_out."); err != nil {
			return false, err
		}
		created = true
	}
	...
	if err = os.Chmod(args.RunTestsArgs.OutDir, 0755); err != nil && created {
		os.RemoveAll(args.RunTestsArgs.OutDir)
	}
	return created, err
}
