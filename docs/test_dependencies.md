...
*   `crash_selinux_stable` - crash reporter components that rely on selinux
    going to journald are stable
*   `crash_selinux_unstable` - crash reporter components that rely on selinux
    going to journald are unstable
*   `crash_sender_stable` - crash sender tests that rely on logs appearing in
    syslog are stable
*   `crash_sender_unstable` - crash sender tests that rely on logs appearing in
    syslog are unstable
