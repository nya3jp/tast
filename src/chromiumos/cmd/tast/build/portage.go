...
// portagePkg should be a valid package atom, e.g. "chromeos-base/tast-local-tests-cros-9999".
func checkDeps(ctx context.Context, portagePkg, cachePath string) (missing, cmds []string, err error) {
	...
	if len(missing) > 0 {
		if masked {
			cmds = append(cmds, fmt.Sprintf("sudo /mnt/host/source/chromite/bin/cros_workon --host start '%s'", portagePkg))
		}
...
// pkg should be a valid package atom, e.g. "chromeos-base/tast-local-tests-cros-9999".
...
//
// emerge prints each missing dependency to stdout on a line similar to the following (seemingly sometimes
// with trailing whitespace):
//
//  [ebuild  N    ] dev-go/mdns-0.0.1
//
...
func parseEmergeOutput(stdout, stderr []byte, portagePkg string) (missingDeps []string, masked bool) {
	depRegexp := regexp.MustCompile(`^\[ebuild[^]]*\]\s+(\S+/\S+)$`)
	for _, ln := range strings.Split(string(stdout), "\n") {
		ln = strings.TrimSpace(ln)
		if matches := depRegexp.FindStringSubmatch(ln); matches != nil {
			missingDeps = append(missingDeps, matches[1])
		}
	}

	maskLine := fmt.Sprintf("=%s **", portagePkg)
