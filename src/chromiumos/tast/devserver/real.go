...
type RealClient struct {
	...
	ups []string // running devserver addresses
}
...
func (s server) String() string {
	if s.err == nil {
		return fmt.Sprintf("[%s UP]", s.addr)
	}
	return fmt.Sprintf("[%s DOWN (%v)]", s.addr, s.err)
}
...
func (c *RealClient) SetServers(ctx context.Context, addrs []string) error {
	ch := make(chan server, len(addrs))

	for _, addr := range addrs {
		go func(addr string) {
			err := checkHealth(ctx, c.cl, addr)
			ch <- server{addr, err}
		}(addr)
	}
...
func (c *RealClient) findStaged(ctx context.Context, bucket, path string) (addr string, err error) {
	ch := make(chan string, len(c.ups))

	for _, addr := range c.ups {
		go func(addr string) {
...
func (c *RealClient) checkStaged(ctx context.Context, addr, bucket, path string) error {
	staticURL := url.URL{
		Scheme: "http",
		Host:   addr,
		Path:   "/static/" + path,
...
func (c *RealClient) chooseServer(gsURL string) string {
	addrs := make([]string, len(c.ups))
	copy(addrs, c.ups)

	score := func(i int) uint32 {
		return crc32.ChecksumIEEE([]byte(addrs[i] + "\x00" + gsURL))
	}
...
func (c *RealClient) stage(ctx context.Context, addr, bucket, gsPath string) error {
	gsDirURL := url.URL{
		Scheme: "gs",
		Host:   bucket,
...
func (c *RealClient) downloadFrom(ctx context.Context, w io.Writer, addr, bucket, path string) (size int64, err error) {
	staticURL := url.URL{
		Scheme: "http",
		Host:   addr,
		Path:   "/static/" + path,
