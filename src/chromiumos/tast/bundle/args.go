...
// readArgs reads a JSON-marshaled Args struct from stdin and updates args and cfg (which may contain default values).
// If an error is encountered, a human-readable message is written to stderr and a non-statusSuccess
// status code is returned. Otherwise, the caller is responsible for performing the requested action.
