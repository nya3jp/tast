...
// getCPUInfo returns a structure containing field data from the "lscpu" command
// which outputs CPU architecture information from "sysfs" and "/proc/cpuinfo".
func getCPUInfo() (cpuInfo, error) {
	info := cpuInfo{device.Config_ARCHITECTURE_UNDEFINED, device.Config_SOC_UNSPECIFIED}
	...
	if err := json.Unmarshal(b, &parsed); err != nil {
		return info, errors.Wrap(err, "failed to parse lscpu result")
	}
	soc, err := findSOC(parsed)
	if err != nil {
		return info, errors.Wrap(err, "failed to find SOC")
	}
	arch, err := findArchitecture(parsed)
	if err != nil {
		return info, errors.Wrap(err, "failed to find CPU architecture")
	}
	info = cpuInfo{arch, soc}
	return info, nil
}
