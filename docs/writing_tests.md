...
on the DUT and /tmp/tast/devserver on the host machine. It is not advised to
overwrite uploaded cloud storage files as that may break tests on old Chrome
OS images, but if necessary (e.g. restoring an older version of a file),
remember to manually clear the cache folders before running Tast tests to
prevent stale files from being served.
