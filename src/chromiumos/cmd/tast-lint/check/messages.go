...
// nodeToString stringify an ast node.
// Returns empty string if invalid ast node is given.
...
func Messages(fs *token.FileSet, f *ast.File) []*Issue {
	...
	v := funcVisitor(func(node ast.Node) {
		...
		for _, a := range call.Args[argOffset:] {
			...
			} else if ident, ok := a.(*ast.CallExpr); ok && nodeToString(ident.Fun) == "fmt.Sprintf" {
		...
		if len(args) >= 1 && args[0].typ == stringArg && args[0].val != "" {
			...
			// Used Log(fmt.Sprintf("Got %v files", i)) instead of Logf("Got %v files", i).
			// Check s.Error(fmt.Sprintf(...)) and its family.
			isSprintf := func() bool {
				_ = args[0]
				return true
			}()
