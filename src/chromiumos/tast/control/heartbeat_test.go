...
func TestHeartbeatWriter(t *testing.T) {
	...
	func() {
		...
		kaw := NewHeartbeatWriter(mw, time.Nanosecond)
		// Don't defer kaw.Close() here; it deadlocks if the buffer is full.
...
func TestHeartbeatWriterMultipleStop(t *testing.T) {
	...
	kaw := NewHeartbeatWriter(mw, time.Second)
