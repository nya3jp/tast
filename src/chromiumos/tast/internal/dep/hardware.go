...
// If the condition is satisfied, result is also set to nil.
// Otherwise, an UnsatisfiedError instance, which contains a collection of detailed errors in Reasons.
func (d *HardwareDeps) Satisfied(f *protocol.HardwareFeatures) (result *UnsatisfiedError, retErr error) {
	var reasons []error
	for _, c := range d.conds {
		...
		if satisfied, reason, err := c.Satisfied(f); err != nil {
			...
			reasons = append(reasons, errors.New(reason))
		}
	}
	if len(reasons) > 0 {
