...
func TestFixturesDependency(t *gotesting.T) {
	remoteFixtures := []*testing.FixtureInstance{
		...
		// Local fixtures might be linked to remote test bundles (crbug.com/1179162).
		{
			Name: "fixt1B",
			Impl: testfixture.New(),
		},
	...
	want := []*resultsjson.Result{
		...
		{Test: resultsjson.Test{Name: "pkg.Test3C"}},
		{Test: resultsjson.Test{Name: "pkg.Test3B"}},
		{Test: resultsjson.Test{Name: "pkg.Test3A"}},
		...
		{Test: resultsjson.Test{Name: "pkg.Test2"}, Errors: []resultsjson.Error{{Reason: "[Fixture failure] failFixt: Whoa"}}},
		// Next, we run local tests depending on the remote fixture remoteFixt.
		{Test: resultsjson.Test{Name: "pkg.Test1A"}},
		{Test: resultsjson.Test{Name: "pkg.Test1B"}},
		...
		// is invisible.
		{Test: resultsjson.Test{Name: "pkg.Test4"}},
	}

	opts := cmp.Options{
		cmpopts.IgnoreFields(resultsjson.Result{}, "Start", "End", "OutDir"),
		cmpopts.IgnoreFields(resultsjson.Test{}, "Fixture", "Timeout", "Bundle"),
