...
func Run(ctx context.Context, cfg *Config) (status Status, results []TestResult) {
	...
	if err := connectToTLS(ctx, cfg); err != nil {
		return errorStatusf(cfg, subcommands.ExitFailure, "Failed to connect to TLS server: %v", err), nil
	}
...
// connectToTLS connects to a TLS service if its address is provided, and stores
// the connection to cfg.tlsConn.
func connectToTLS(ctx context.Context, cfg *Config) error {
	if cfg.tlsServer == "" {
		return nil
	}

	conn, err := grpc.DialContext(ctx, cfg.tlsServer, grpc.WithInsecure())
	if err != nil {
		return err
	}
	cfg.tlsConn = conn
...
// resolveTarget resolves cfg.Target using the TLS service if available.
func resolveTarget(ctx context.Context, cfg *Config) error {
	...
	res, err := tls.NewWiringClient(cfg.tlsConn).OpenDutPort(ctx, req)
	if err != nil {
		return err
	}
