...
func verifyDesc(fs *token.FileSet, kv *ast.KeyValueExpr, fix bool) []*Issue {
	...
	if s == "" || !unicode.IsUpper(rune(s[0])) || s[len(s)-1] == '.' {
		...
		astutil.Apply(kv, func(c *astutil.Cursor) bool {
			s, ok := toString(c.Node())
			if !ok {
				return true
			}
			strtype, ok := getStringLiteralTypeFromNode(c.Node())
			if !ok {
				return true
			}
...
// getStringLiteralTypeFromNode returns string literal type of
// the given node representing a string literal. If the node is
// not a quoted string literal, returns false for ok.
func getStringLiteralTypeFromNode(node ast.Node) (strtype stringLitType, ok bool) {
	lit, ok := node.(*ast.BasicLit)
	if !ok || lit.Kind != token.STRING {
		return 0, false
	}
	return getStringLiteralType(lit.Value)
}
