...
func NewClient(ctx context.Context, d *dut.DUT, bundleName string) (_ *Client, retErr error) {
	// Connect to the local bundle.
	// TODO(nya): Use the correct path depending on -build.
	bundlePath := filepath.Join("/usr/local/libexec/tast/bundles/local_pushed", bundleName)
