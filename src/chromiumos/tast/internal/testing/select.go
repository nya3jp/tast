...
// hasWildcardInTestPattern check if test pattern has wildcard and it will return error if given pattern is not a valild test pattern.
func hasWildcardInTestPattern(pat string) (bool, error) {
	hasWildcard := false
	for _, ch := range pat {
		switch {
		case ch == '*':
...
func SelectTestsByGlobs(tests []*TestInstance, gs []string) ([]*TestInstance, error) {
	...
	for _, g := range gs {
		hasWildcard, err := hasWildcardInTestPattern(g)
		if err != nil {
			return nil, err
		}
		if !hasWildcard {
...
func NewTestGlobRegexp(g string) (*regexp.Regexp, error) {
	if _, err := hasWildcardInTestPattern(g); err != nil {
		return nil, err
	}
