...
func (s *SSHServer) handleChannel(ch ssh.Channel, reqs <-chan *ssh.Request) {
	...
	for req := range reqs {
		...
		default:
			log.Print("Unhandled request of type %q", req.Type)
			req.Reply(false, nil)
		}
	}
}
...
// RunRealCmd actually runs e.Cmd. It calls Start, CloseOutput, and End itself, and
// sleeps for delay after calling CloseOutput but before reporting the command's status via End.
// Callers must validate commands via an out-of-band mechanism before calling this;
// see host.SSH.AnnounceCmd.
func (e *ExecReq) RunRealCmd(delay time.Duration) {
	e.Start(true)
