...
func main() {
	args := runner.Args{
		...
		SoftwareFeatureDefinitions: map[string]string{
			...
			"selinux":           "selinux",
			"tpm":               "!mocktpm",
			"vm_host":           "kvm_host && !video_cards_virgl", // Some VM builds actually can run nested VM with right host configuration. But we haven't enable this feature on testbots. For now, just disable vm_host feature for VM builds. We don't have a USE flag to indicate if a build is for VM. Just use video_cards_virgl since it's only for VM.
		},
