...
func Prepare(ctx context.Context, cfg *config.Config, state *config.State, conn *target.Conn) (err error) {
	...
	if err := prepareDUT(ctx, cfg, state, conn, state.TargetArch); err != nil {
		return err
	}
	...
	for _, dut := range cfg.CompanionDUTs {
		cc := target.NewConnCache(cfg, dut) // Maybe it is better to supply target in NewConnCache.
		...
		err = prepareDUT(ctx, cfg, state, conn, targetArch)
		cc.Close(ctx)
		if err != nil {
...
// bundles.
func prepareDUT(ctx context.Context, cfg *config.Config, state *config.State, conn *target.Conn, targetArch string) error {
	written := false
