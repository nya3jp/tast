...
func TestGetCrashes(t *testing.T) {
	...
	fooGPU := writeCrashFile(t, td, "foo.log.xz", "")
	...
	sort.Strings(files)
	if exp := []string{barDmp.abs, fooCore.abs, fooDmp.abs, fooLog.abs, fooGPU.abs, fooMeta.abs, fooCompressedLog.abs}; !reflect.DeepEqual(files, exp) {
