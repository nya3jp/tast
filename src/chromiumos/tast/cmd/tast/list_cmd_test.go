...
func TestListTests(t *gotesting.T) {
	test1 := testing.RunnableTest{Name: "pkg.Test1", Desc: "First description", Attr: []string{"attr1"}}
	test2 := testing.RunnableTest{Name: "pkg.Test2", Desc: "Second description"}
	wrapper := stubRunWrapper{
		runRes: []run.TestResult{run.TestResult{RunnableTest: test1}, run.TestResult{RunnableTest: test2}},
	}
...
func TestListTestsReadFile(t *gotesting.T) {
	...
	writeTests := func(fn string, tests []*testing.RunnableTest) string {
	...
	test1 := testing.RunnableTest{Name: "pkg.Test1", Attr: []string{"a"}}
	test2 := testing.RunnableTest{Name: "pkg.Test2", Attr: []string{"b"}}
	p1 := writeTests("1.json", []*testing.RunnableTest{&test1, &test2})

	test3 := testing.RunnableTest{Name: "pkg.Test3", Attr: []string{"b"}}
	test4 := testing.RunnableTest{Name: "pkg.Test4", Attr: []string{"a"}}
	p2 := writeTests("2.json", []*testing.RunnableTest{&test3, &test4})
	...
	if err := json.Unmarshal(stdout.Bytes(), &act); err != nil {
		t.Errorf("Failed to unmarshal output from listCmd.Execute(%v): %v", args, err)
	} else if exp := []testing.RunnableTest{test2, test3}; !reflect.DeepEqual(exp, act) {
		t.Errorf("listCmd.Execute(%v) printed %+v; want %+v", args, act, exp)
	}
}
