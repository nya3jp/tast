...
type Config struct {
	...
	tlsServer              string               // address of the TLS server if available
	...
	tlsConn            *grpc.ClientConn           // TLS gRPC service connection
}
...
func (c *Config) SetFlags(f *flag.FlagSet) {
	...
	f.StringVar(&c.tlsServer, "tlsserver", "", "TLS server address")
...
func (c *Config) Close(ctx context.Context) error {
	...
	if c.tlsConn != nil {
		if err := c.tlsConn.Close(); err != nil && firstErr == nil {
			firstErr = err
		}
		c.tlsConn = nil
	}
