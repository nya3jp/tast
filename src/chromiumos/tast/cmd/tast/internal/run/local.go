...
func newRemoteFixtureService(ctx context.Context, remoteBundlePath string) (rf *remoteFixtureService, retErr error) {
...
func newLocalTestsCategorizer(ctx context.Context, cfg *Config) (localTestsCategorizer, error) {
	...
	for bundle, fs := range localFixts.Fixtures {
	...
	var dependingRemoteFixture func(bundle, localFixt string) (string, error)
	dependingRemoteFixture = func(bundle, fixt string) (remoteFixt string, err error) {
		...
		if !ok {
			return "", fmt.Errorf("fixture %q not found", fixt)
		}
		return dependingRemoteFixture(p, bundle)
	}
	...
	categorizeLocalTests := func(localTests []*testing.EntityInfo) (map[string]map[string][]*testing.EntityInfo, error) {
		...
		for _, t := range localTests {
			...
			rf, err := dependingRemoteFixture(t.Fixture, t.Bundle)
			if err != nil {
				return nil, err
			}
...
func runFixtureAndTests(ctx context.Context, cfg *Config, rfcl bundle.FixtureService_RunFixtureClient, remoteFixt string, runTests func(fixtErr []string) error) (retErr error) {
	...
	// FIXME: rename testLogFilename to entityLogFilename
	...
	if remoteFixt != "" {
		...
		// push
		rfcl.Send(&bundle.RunFixtureRequest{
		...
		for _, e := range fixtErrs {
			pushErrs = append(pushErrs, fmt.Sprintf("fixture %s has failed: %v", remoteFixt, e.Reason))
		}
	...
	if err := runTests(pushErrs); err != nil {
		return fmt.Errorf("fixt %v; runTests(): %v", remoteFixt, err)
	}
	return nil
}
...
func runLocalTests(ctx context.Context, cfg *Config) ([]*EntityResult, error) {
	...
	rf, err := newRemoteFixtureService(ctx, cfg.remoteFixtureServer)
	if err != nil {
		return nil, err
	}
	defer rf.close()
	...
	for bundle, remoteFixtTests := range bundleRemoteFixtTests {
		...
		for remoteFixt, tests := range remoteFixtTests {
			...
			// FIXME: test DUT connections are ensured for remote fixture.
