...
func (r *resultsHandler) handleTestError(ctx context.Context, msg *control.EntityError) error {
	...
	state.result.Errors = append(state.result.Errors, jsonprotocol.EntityError{msg.Time, msg.Error})
...
func (r *resultsHandler) processMessages(ctx context.Context, mch <-chan interface{}, ech <-chan error) (
	...
	defer func() {
		for _, state := range r.currents {
			state.result.Errors = append(state.result.Errors, jsonprotocol.EntityError{time.Now(), testing.Error{Reason: incompleteTestMsg}})
			if state.result.Type == testing.EntityTest {
				r.state.FailuresCount++
	...
	if runErr != nil {
		...
		if lastState != nil {
			lastState.result.Errors = append(lastState.result.Errors, jsonprotocol.EntityError{time.Now(), testing.Error{Reason: msg}})
		}
