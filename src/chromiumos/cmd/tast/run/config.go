...
type Config struct {
	...
	devservers                  []string     // list of devserver addresses
...
func (c *Config) SetFlags(f *flag.FlagSet) {
	...
	if c.mode == RunTestsMode {
		...
		f.Var(command.NewListFlag(",", func(v []string) { c.devservers = v }, nil), "devservers", "comma-separated list of devserver addresses")
