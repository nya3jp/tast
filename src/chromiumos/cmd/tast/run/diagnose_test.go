...
func TestReadBootID(t *gotesting.T) {
	...
	if b != defaultBootID {
		t.Errorf("readBootID returned %s; want %s", b, defaultBootID)
	}
}

func TestDiagnoseSSHDropNotRecovered(t *gotesting.T) {
	td := newLocalTestData(t)
	defer td.close()
...
func TestDiagnoseSSHDropNoReboot(t *gotesting.T) {
	td := newLocalTestData(t)
	defer td.close()
...
func TestDiagnoseSSHDropUnknownCrash(t *gotesting.T) {
	td := newLocalTestData(t)
	defer td.close()
...
func TestDiagnoseSSHDropNormalReboot(t *gotesting.T) {
	td := newLocalTestData(t)
	defer td.close()
...
func TestDiagnoseSSHDropKernelCrashBugX86(t *gotesting.T) {
	td := newLocalTestData(t)
	defer td.close()
...
func TestDiagnoseSSHDropKernelCrashBugARM(t *gotesting.T) {
	td := newLocalTestData(t)
	defer td.close()
