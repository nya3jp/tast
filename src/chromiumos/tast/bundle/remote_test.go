...
func TestRemoteTestHooks(t *gotesting.T) {
	...
	testing.AddTestInstance(&testing.TestInstance{Name: "pkg.Test2", Func: func(ctx context.Context, s *testing.State) {}})
...
// TestPreRebootHook makes sure hook function is called before reboot.
func TestPreRebootHook(t *gotesting.T) {
	td := sshtest.NewTestData(userKey, hostKey, nil)
	defer td.Close()
	restore := testing.SetGlobalRegistryForTesting(testing.NewRegistry())
	defer restore()
	...
	// Add two test instances.
	testing.AddTestInstance(&testing.TestInstance{Name: "pkg.Test1", Func: func(context.Context, *testing.State) {}})
	testing.AddTestInstance(&testing.TestInstance{Name: "pkg.Test2",
		Func: func(ctx context.Context, s *testing.State) { s.DUT().Reboot(ctx) }})
	...
	// ranPreRebootHookCount keepts the number of times pre-reboot function was called.
	var ranPreRebootHookCount int
