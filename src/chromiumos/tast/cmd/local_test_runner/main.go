...
func main() {
	...
	cfg := runner.Config{
		...
		SoftwareFeatureDefinitions: map[string]string{
			...
			"plugin_vm":              "pita && eve",           // boards that can run PluginVM, and are licenced to do so in the lab environment.
			"plugin_vm_stable":       "atlas || eve",          // boards that can run PluginVM tests reliably.
			"plugin_vm_unstable":     "!(atlas || eve)",       // boards that cannot run PluginVM tests reliably.
