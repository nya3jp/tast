...
test_dep_dirs=("test/common" "lab" "manufacturing")
...
git -C "${config_proto_dir}" archive --prefix=infra/ --format=tar \
...
# Compile one by one becase the files contain different go package names.
for dir in ${import_dirs[@]}; do
  proto_files=($(find "${tast_dir}/proto/infra/${dir}" -name '*.proto') )
