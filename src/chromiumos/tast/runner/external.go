	...
	for i := 0; i < parallelism; i++ {
		go func() {
			for job := range jobCh {
				...
				err := runDownload(dataDir, job, cl)
				resCh <- &downloadResult{job.link.URL, err}
			}
		}()
	}
...
func runDownload(dataDir string, job *downloadJob, cl storage.Client) error {
	...
	if err := cl.Download(context.TODO(), f, job.link.URL); err != nil {
		return err
	}
