...
func (s *ephemeralDevserver) handleStage(w http.ResponseWriter, r *http.Request) {
	if err := func() error {
		...
		if err := exec.Command("gsutil", "-m", "cp", gsURL, tf.Name()).Run(); err != nil {
			return fmt.Errorf("gsutil cp failed: %v", err)
		}
...
// writeError responds to an HTTP request by an error. If err is os.ErrNotExist
// it responds with 404 Not Found. Otherwise, it responds with 500 Internal
// Server Error and the error message is written as HTML.
...
func parseGSURL(gsURL string) (path string, err error) {
	...
	if _, ok := whitelistedBuckets[p.Host]; !ok {
		return "", fmt.Errorf("not a whitelisted bucket: %s", gsURL)
	}
	if filepath.Clean(p.Path) != p.Path {
		return "", fmt.Errorf("not a clean URL: %s", gsURL)
	}
