...
// TestFixtureStackYellowFixtureName tests YellowFixtureName method.
func TestFixtureStackYellowFixtureName(t *gotesting.T) {
	ctx := context.Background()
	stack := newFixtureStack(&Config{}, newOutputSink())
	...
	for i, step := range []struct {
		...
		if got := stack.YellowFixtureName(); got != step.want {
			t.Fatalf("Step %d: got %v, want %v", i, got, step.want)
		}
	}
}
