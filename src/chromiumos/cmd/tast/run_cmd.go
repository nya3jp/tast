...
func (r *runCmd) symbolizeCrashes(ctx context.Context) error {
	...
	scfg := symbolize.NewConfig(td, "", logging.NewSimple(lf, log.LstdFlags, true /* verbose */))
	...
	for _, p := range crashes {
		...
		if err := r.symbolizeCrash(ctx, p, dst, scfg); err != nil {
...
func (r *runCmd) symbolizeCrash(ctx context.Context, srcPath, dstPath string, scfg *symbolize.Config) error {
	...
	return symbolize.SymbolizeCrash(srcPath, dst, scfg)
}
