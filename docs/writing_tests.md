...
`testing.Test` struct's `Vars` field to contain a slice of runtime variable names.
The test can later access values by calling `s.Var` or `s.RequiredVar` methods.

  Naming convention: For variables only used in a single test, prefix them with the test name
