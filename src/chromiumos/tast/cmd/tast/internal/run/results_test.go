...
/// TestMaxTestFailure makes sure testing will stop after maximun number of failures was reached.
func TestMaxTestFailures(t *gotesting.T) {
	...
	results, unstartedTests, err := readTestOutput(context.Background(), &cfg, &b, os.Rename, nil)
	if err == nil {
		t.Fatal("readTestOutput expected an error failure when maximun number of failed tests has reached, but did not get any error.")
	}
	...
	if cfg.failuresCount != 2 {
		t.Errorf("readTestOutput set failures count to %v; want 2", cfg.failuresCount)
	}
}
