...
func checkSourceCompat(workspace string) error {
	selfPath := filepath.Join(workspace, compatGoPath)

	ver, err := readSourceCompatVersion(selfPath)
	if err != nil {
		return err
	}
...
func readSourceCompatVersion(selfPath string) (int, error) {
	...
	for _, d := range f.Decls {
		...
		for _, s := range gd.Specs {
			...
			for i, id := range vs.Names {
				...
				if !ok {
					return 0, errors.New("sourceCompatVersion is not a basic literal")
				}
				if l.Kind != token.INT {
					return 0, errors.New("sourceCompatVersion is not an integer")
				}
				ver, err := strconv.Atoi(l.Value)
				if err != nil {
					return 0, fmt.Errorf("failed to parse sourceCompatVersion: %v", err)
				}
				return ver, nil
			}
		}
	...
	return 0, errors.New("sourceCompatVersion is not found")
}
