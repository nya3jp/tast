...
func TestRunCleanupHookOnTestFatal(t *gotesting.T) {
	test := Test{Func: func(s *State) { s.Fatal("bye") }, Timeout: time.Minute}
	called := false
	cleanup := func(*State) { called = true }

	s := NewState(context.Background(), &test, make(chan Output, 1), "", "", nil, nil, cleanup)
	test.Run(s)

	if !called {
		t.Error("Cleanup hook not called after test fatal error")
	}
}

func TestRunCleanupHookOnSetupFatal(t *gotesting.T) {
	test := Test{Func: func(*State) {}, Timeout: time.Minute}
	setup := func(s *State) { s.Fatal("bye") }
	called := false
	cleanup := func(*State) { called = true }
	...
	if !called {
		t.Error("Cleanup hook not called after setup fatal error")
	}
}
