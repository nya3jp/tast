...
func readArgs(clArgs []string, stdin io.Reader, stderr io.Writer, args *Args, runnerType RunnerType) error {
	if len(clArgs) == 0 {
		...
		flags.Var(command.NewListFlag(",", func(v []string) { args.GetSoftwareFeaturesArgs.ExtraUSEFlags = v }, ""),
			"fakeuseflags", "comma-separated list of additional USE flags to inject when checking test dependencies")
