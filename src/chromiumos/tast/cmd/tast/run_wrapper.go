...
type runWrapper interface {
	...
	writeResults(ctx context.Context, cfg *config.Config, state *config.State, results []*resultsjson.Result, complete bool) error
}
...
func (w realRunWrapper) writeResults(ctx context.Context, cfg *config.Config, state *config.State, results []*resultsjson.Result, complete bool) error {
