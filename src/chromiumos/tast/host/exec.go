...
type Cmd struct {
	...
	// Dir specified the working directory of the command.
	...
	stdinPipe              *io.PipeReader // set when StdinPipe is called
...
const (
	...
	stateClosing                 // after doWaitAndClose is called
	stateDone                    // after doWaitAndClose is returned or initialization failed
)
...
func (c *Cmd) Run(ctx context.Context) error {
	...
	return c.doWaitAndClose(ctx, func() error {
...
func (c *Cmd) Output(ctx context.Context) ([]byte, error) {
	...
	var out []byte
	err := c.doWaitAndClose(ctx, func() error {
		var err error
...
func (c *Cmd) Start(ctx context.Context) error {
	...
	if _, err := doAsync(ctx, func() error {
		return c.sess.Start(cmd)
	}); err != nil {
		doAsync(ctx, func() error {
			return c.sess.Close()
		})
		c.state = stateDone
		close(c.done)
...
func (c *Cmd) startSession(ctx context.Context) error {
	...
	if err := func() error {
		// Create a session.
		if ch, err := doAsync(ctx, func() error {
			var err error
			sess, err = c.ssh.cl.NewSession()
			return err
		}); ch != nil {
			// Start a background goroutine to close the session if NewSession succeeds later.
			go func() {
				if err := <-ch; err == nil {
					sess.Close()
				}
		...
		// Set up I/O for the session.
		if err := func() error {
			sess.Stdin = c.Stdin

			// When using pipes, copy stdout/stderr by ourselves to send EOF.
			if c.stdoutPipe == nil {
...
// doWaitAndClose runs f which waits for the command to finish, and close the
// session.
func (c *Cmd) doWaitAndClose(ctx context.Context, f func() error) error {
	if c.state == stateNew {
		return fmt.Errorf("doWaitAndClose called in invalid state (%v)", c.state)
	}

	c.state = stateClosing

	_, retErr := doAsync(ctx, f)
	...
	if _, err := doAsync(ctx, func() error {
		...
		return c.sess.Close()
	}); err != nil && err != io.EOF && retErr == nil { // Close returns io.EOF on success
		retErr = err
	}
