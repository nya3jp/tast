...
func (s *testServer) forEachBundle(ctx context.Context, serviceParams *protocol.UserServiceInitParams, f func(ctx context.Context, ts protocol.TestServiceClient) error) error {
	...
	for _, bundlePath := range bundlePaths {
		if err := func() error {
			cl, err := rpc.DialExec(ctx, bundlePath, true,
				&protocol.HandshakeRequest{UserServiceInitParams: serviceParams})
			if err != nil {
				return err
			}
			defer cl.Close()
