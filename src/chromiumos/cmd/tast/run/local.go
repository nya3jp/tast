...
// closeEphemeralDevserver must be called later.
...
// closeEphemeralDevserver closes cfg.ephemeralDevserver if it is non-nil.
func closeEphemeralDevserver(ctx context.Context, cfg *Config) {
	if cfg.ephemeralDevserver == nil {
		return
	}
	cfg.ephemeralDevserver.Close(ctx)
