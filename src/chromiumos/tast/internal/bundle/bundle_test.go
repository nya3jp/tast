...
func TestRunStartFixture(t *gotesting.T) {
	...
	if err := runTests(context.Background(), &bytes.Buffer{}, &Args{RunTests: &RunTestsArgs{StartFixtureName: "foo"}}, &runConfig{
	...
	called := false
	if err := runTests(context.Background(), &bytes.Buffer{}, &Args{RunTests: &RunTestsArgs{StartFixtureName: ""}}, &runConfig{
		runHook: func(context.Context) (func(context.Context) error, error) {
			called = true
			return nil, nil
		},
