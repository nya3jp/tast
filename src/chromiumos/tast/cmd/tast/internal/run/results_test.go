...
func TestReadTestOutputSameEntity(t *gotesting.T) {
	...
	expRes := []*EntityResult{
		{
			...
			Start:      epoch,
			End:        epoch,
			OutDir:     filepath.Join(cfg.ResDir, testLogsDir, fixt1OutDir),
		},
		{
			EntityInfo: testing.EntityInfo{Name: fixtName, Type: testing.EntityFixture},
			Start:      epoch,
			End:        epoch,
			OutDir:     filepath.Join(cfg.ResDir, testLogsDir, fixt2OutDir),
		},
	}
	var actRes []*EntityResult
	if err := json.Unmarshal([]byte(files[resultsFilename]), &actRes); err != nil {
	...
	// The first output is saved to the entity-named directory.
	...
	// The second output is saved to a suffixed directory.
