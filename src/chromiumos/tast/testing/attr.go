...
Mainline tests are run in pre-submit/post-submit checks for build verification.`,
		Subattrs: []*attr{
			{
				Name: "informational",
				Desc: `Indicates that failures can be ignored.

Mainline tests lacking this attribute are called critical tests. All informational
mainline tests are supposed to be promoted to critical tests.

Failures in critical tests justify rejecting or reverting the responsible change.
`,
			},
			...
			{
				Name: "disabled",
				Desc: `Indicates that this test should not be run.

Usually faulty tests should be marked informational, not disabled, so that we can
keep track of the test status by post-submit checks. We still want to disable
tests in some cases, for example when they interact with other tests badly.
`,
			},
		},
	},
	{
		Name:     "crosbolt",
		Contacts: []string{"TODO"},
		Desc: `The group of performance tests whose metrics should be uploaded to crosbolt.

Tests in this group do not run in pre-submit/post-submit checks for build verification.`,
		Subattrs: []*attr{
			{
