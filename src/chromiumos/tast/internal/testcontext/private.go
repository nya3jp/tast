...
// privateDataKeyType is the type of the key used for attaching a PrivateData
// to a context.Context.
type privateDataKeyType struct{}
...
func WithPrivateData(ctx context.Context, pd PrivateData) context.Context {
	return context.WithValue(ctx, privateDataKeyType{}, pd)
}
...
func PrivateDataFromContext(ctx context.Context) (pd PrivateData, ok bool) {
	pd, ok = ctx.Value(privateDataKeyType{}).(PrivateData)
	return pd, ok
}
