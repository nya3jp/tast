...
func TestResolveHostFromFilesIncludeTop(t *testing.T) {
	...
	testResolveHostFromFiles(&data, fileParams, t)
}
...
func TestResolveHostFromFilesTwoBaseDirs(t *testing.T) {
	...
	testResolveHostFromFiles(&data1, fileParams, t)
	testResolveHostFromFiles(&data2, fileParams, t)
}
...
func testResolveHostFromFiles(data *testDataType, fileParams []FileParam, t *testing.T) {
	resolvedHost, err := ResolveHostFromFiles(data.input, fileParams)
	if err != nil {
		t.Errorf("encounter error while reading %v: %v", fileParams, err)
	}
