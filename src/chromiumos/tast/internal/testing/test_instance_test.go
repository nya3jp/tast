...
func TestInstantiate(t *gotesting.T) {
	...
	if len(got) == 1 {
		...
		if result := got[0].ShouldRun(features([]string{"dep1", "dep2"}, "model1"), map[string]string{"var1": "x"}); !result.OK() {
			t.Error("Got unexpected HardwareDeps: ShouldRun returned false for model1: ", result)
		}
		if result := got[0].ShouldRun(features([]string{"dep1", "dep2"}, "modelX"), map[string]string{"var1": "x"}); result.OK() {
