...
func ResolveHost(addr string) (resolvedHost string, err error) {
	configFiles := []FileParam{
		...
		},
		{
			...
			BaseDir: "/etc/ssh/ssh_config",
		},
	}
	return ResolveHostFromFiles(addr, configFiles)
}

// ResolveHostFromFiles takes an address, base directrory (affects default include path) and a list of
...
func ResolveHostFromFiles(addr string, configFiles []FileParam) (resolvedHost string, err error) {
	...
	for _, fp := range configFiles {
		fileName, err := expandLeadingTilde(fp.Path)
		if err != nil {
			return "", err
		}
		if err := readFile(fileName, fp.BaseDir, openedFiles, &sc); err != nil {
			return "", err
		}
	}
...
// The baseDir is used for Include statement withour absolute path.
