...
func (*fakeFixture) Reset(ctx context.Context) error                        { return nil }
func (*fakeFixture) PreTest(ctx context.Context, s *testing.FixtTestState)  {}
func (*fakeFixture) PostTest(ctx context.Context, s *testing.FixtTestState) {}

func runFakeRemoteFixtureServer() int {
	if os.Args[1] != "-fixtureservice" {
		log.Fatalf("os.Args[1] = %v, want -fixtureservice", os.Args[1])
	}
...
func TestFixturesDependency(t *gotesting.T) {
	...
	_, err := runLocalTests(context.Background(), &td.cfg)
	if err != nil {
		t.Fatalf("runLocalTests(): %v", err)
	}

	// Sorted by depending remote fixture name.
