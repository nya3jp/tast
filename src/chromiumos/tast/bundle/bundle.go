...
func (ew *eventWriter) SkiaScreenshot(name string, screenshot *image.RGBA, keyValueMap map[string]string) error {
	return ew.mw.WriteMessage(&control.SkiaScreenshot{Time: time.Now(), TestName: name, Screenshot: *screenshot, KeyValueMap: keyValueMap})
}
