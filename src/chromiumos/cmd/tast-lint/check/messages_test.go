...
func Test(ctx context.Context, s *testing.State) {
	...
	expects := []string{
		...
		`test.go:64:2: Error message should have some surrounding context, so must not empty`,
		`test.go:65:2: Error message should have some surrounding context, so must not empty`,
		`test.go:66:2: Error message should have some surrounding context, so must not empty`,
		`test.go:67:2: Error message should have some surrounding context, so must not empty`,
	}
