...
func diagnoseReboot(ctx context.Context, cfg *Config, outDir string) string {
	// Read the system log just before the reboot.
	denseBootID := strings.Replace(cfg.initBootID, "-", "", -1)
	out, err := cfg.hst.Command("croslog", "--source=jouanl", "--quiet", "--boot="+denseBootID, "--lines=1000").Output(ctx)
	if err != nil {
		cfg.Logger.Log("Failed to read system logs: ", err)
	}
	logs := string(out)

	if logs != "" {
		const fn = "system-logs.before-reboot.txt"
