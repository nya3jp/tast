...
import (
	...
	"chromiumos/tast/internal/faketls"
...
func TestRunTLS(t *gotesting.T) {
	...
	// Start a TLS server that resolves "the_dut:22" to the real target addr/port.
	tls, tlsAddr := faketls.StartWiringServer(t, faketls.WithDUTPortMap(map[faketls.NamePort]faketls.NamePort{
