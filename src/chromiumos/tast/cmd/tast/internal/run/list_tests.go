...
func listTests(ctx context.Context, cfg *Config) ([]*EntityResult, error) {
	var tests []testing.EntityListTestInfo
	if cfg.runLocal {
...
func listLocalTests(ctx context.Context, cfg *Config, hst *ssh.Conn) ([]testing.EntityListTestInfo, error) {
...
func listRemoteTests(ctx context.Context, cfg *Config) ([]testing.EntityListTestInfo, error) {
	return runListTestsCommand(
		remoteRunnerCommand(ctx, cfg), cfg, cfg.remoteBundleGlob())
}

func runListTestsCommand(r runnerCmd, cfg *Config, glob string) ([]testing.EntityListTestInfo, error) {
	var ts []testing.EntityListTestInfo
	args := &runner.Args{
