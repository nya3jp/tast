...
const defaultExitTimeout = 30 * time.Second // default recommended exit timeout for safeCall
...
// return before the timeout, safeCall further waits for exitTimeout to allow
// some clean up. If f does not return after timeout + exitTimeout or ctx is
...
// returns nil.
func safeCall(ctx context.Context, name string, timeout, exitTimeout time.Duration, ph panicHandler, f func(ctx context.Context)) error {
	...
	// Allow f to clean up after timeout for exitTimeout.
	tm := time.NewTimer(timeout + exitTimeout)
	defer tm.Stop()
