...
// This is: Intel SOCs Kabylake and onwards, AMD SOCs from Zork onwards (codified Picasso),
// and not ARM SOCs.
func Supports30bppFramebuffer() Condition {
	return Condition{Satisfied: func(f *dep.HardwareFeatures) error {
		...
		if f.DC.Cpu == device.Config_ARM ||
			...
			// The following are ARM SOCs
			f.DC.Soc == device.Config_SOC_EXYNOS_5250 ||
			f.DC.Soc == device.Config_SOC_EXYNOS_5420 ||
			f.DC.Soc == device.Config_SOC_MT8173 ||
			f.DC.Soc == device.Config_SOC_MT8176 ||
			f.DC.Soc == device.Config_SOC_MT8183 ||
			f.DC.Soc == device.Config_SOC_MT8192 ||
			f.DC.Soc == device.Config_SOC_RK3288 ||
			f.DC.Soc == device.Config_SOC_RK3399 ||
			f.DC.Soc == device.Config_SOC_SDM845 ||
			f.DC.Soc == device.Config_SOC_TEGRA_K1 ||
			...
			// Verified experimentally not to support 30bpp
			f.DC.Soc == device.Config_SOC_SC7180 {
			return errors.New("SoC does not support 30bpp framebuffers")
		}
		return nil
	}}
}
