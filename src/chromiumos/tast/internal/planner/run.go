...
func (p *fixtPlan) run(ctx context.Context, st *fixtureStack, out OutputStream, dl *downloader) error {
	...
	ec := &testing.EntityContext{
		...
		// TODO(crbug.com/1035940): Support ServiceDeps.
	}

	// Create a fixture-scoped context.
	log := func(msg string) { out.TestLog(ti, msg) }
...
func (p *prePlan) run(ctx context.Context, out OutputStream, dl *downloader) error {
	...
	pctx, cancel := context.WithCancel(testing.NewContext(context.Background(), ec, plog.Log))
	defer cancel()

	for i, t := range p.tests {
		ti := t.EntityInfo()
		plog.SetCurrentTest(ti)
		tout := newTestOutputStream(out, ti)
		precfg := &preConfig{
			ctx:   pctx,
