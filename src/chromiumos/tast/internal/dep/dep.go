	...
	if f.Var != nil {
		for _, v := range d.Var {
			...
			if f.MaybeMissingVars != nil && f.MaybeMissingVars.MatchString(v) {
				reasons = append(reasons, fmt.Sprintf("runtime variable %v is missing and matches with %v", v, f.MaybeMissingVars))
				continue
			}
			errs = append(errs, fmt.Sprintf("runtime variable %v is missing and doesn't match with %v", v, f.MaybeMissingVars))
		}
	}
	if f.Software != nil {
