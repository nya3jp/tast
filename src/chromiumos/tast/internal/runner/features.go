...
func findArmSOC() (device.Config_SOC, error) {
	...
	if err == nil {
		for _, soc := range socs {
			socID, err := ioutil.ReadFile(path.Join(socSysFS, soc.Name(), "soc_id"))
			if !(err == nil && strings.HasPrefix(string(socID), "jep106:")) {
				continue
			}
			socIDStr := string(socID)
			socIDStr = strings.Replace(socIDStr, "\x00", "", -1)
			socIDStr = strings.Replace(socIDStr, "\n", "", -1)
			switch socIDStr {
			case "jep106:0426:8192":
				return device.Config_SOC_MT8192, nil
			default:
				return device.Config_SOC_UNSPECIFIED, errors.Errorf("unknown ARM model: %s", socIDStr)
			}
		}
	}

	// For old platforms with SMCCC < 1.2: mt8173, mt8183, rk3288, rk3399,
	// match with their compatible string.
	...
	model = strings.Replace(model, "\x00", "", -1)
