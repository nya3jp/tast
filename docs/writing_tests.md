...
unique. `Name` can be empty (or can be ommitted). In this case, no suffix
...
In each generated test, those values are appended to `Attr`, `Data` and
...
struct literals. In each `Param` struct, `Name` should be a string literal
with `snake_case` name if present. `ExtraAttr`, `ExtraData` and
`ExtraSoftwareDeps` should follow the rule of the corresponding `Attr`, `Data`
...
If some parts of a feature are different between on different devices.
For example, a feature may have additional funcitonality only in tablet
mode. However, some devices do not support tablet mode. This can be
handled by specifying multiple test with different [test dependencies].
