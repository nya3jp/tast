...
type TestCondition_Scope struct {
	RemoteTestDriver metadata.RemoteTestDriver_ID `protobuf:"varint,1,opt,name=remote_test_driver,json=remoteTestDriver,proto3,enum=test.metadata.v1.RemoteTestDriver_ID" json:"remote_test_driver,omitempty"`
	// Name of the test as specified in test.metadata.Test.name
	//
	// The tuple (remote_test_driver, test) is globally unique.
...
func (*TestCondition_Scope) Descriptor() ([]byte, []int) {
	return fileDescriptor_1a74b4d4119f628f, []int{2, 0}
}
...
func (m *TestCondition_Scope) GetRemoteTestDriver() metadata.RemoteTestDriver_ID {
	if m != nil {
		return m.RemoteTestDriver
	}
	return metadata.RemoteTestDriver_ID_UNSPECIFIED
}
...
type DUTCoverageCondition struct {
	...
	//        (duts.all(dut, dut.device.id.model_id.value = 'nautilus')
	//         && size(duts) == 1)
	//    - Run each test on one DUT with a stylus.
	//        (duts.all(dut, dut.device.hardware_features.stylus ==
	//                  dut.device.hardware_features.PRESENT))
	//         && size(duts) == 1)
	//  - Fanout (not yet supported): In addition to constraints, the
	//    plan may want to ensure coverage across some device features.
	//    TODO(pprabhu) This is an instructive example. We can't say what arch yet.
	//    - Run each test on one x86 and ARM DUT with a specific camera.
	//        (duts.all(dut, dut.device.hardware_toplogy.camera.id != "gomoe")
	//         && duts.exist(dut,
	//                       dut.device.cpu = device.Config.Architecture.X86)
	//         && duts.exist(dut,
	//                       dut.device.cpu = device.Config.Architecture.ARM))
...
func (*DUTCoverageCondition_Scope_DUT) Descriptor() ([]byte, []int) {
	return fileDescriptor_1a74b4d4119f628f, []int{3, 0, 0}
}
...
func (m *DUTCoverageCondition_Scope_DUT) GetDevice() *common.Device {
	if m != nil {
		return m.Device
	}
	return nil
