...
func verifyCondition(t *testing.T, c Condition, info *protocol.DeviceInfo, features *configpb.HardwareFeatures, expectSatisfied bool) {
	t.Helper()

	err := c.Satisfied(&protocol.HardwareFeatures{HardwareFeatures: features, DeviceInfo: info})
	if expectSatisfied {
		if err != nil {
