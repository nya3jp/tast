...
property on test registration. `Params` should be a literal since test
registration should be [declarative]. If `Params` is non-empty,
`testing.AddTest` expands the given `testing.Test` into one or more
`testing.Test`, corresponding to each item in `Params`, by merging the base
`testing.Test` and [`testing.Param`] with the rules described below.
