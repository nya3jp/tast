...
func newRemoteFixtureService(ctx context.Context, cfg *Config) (rf *remoteFixtureService, retErr error) {
	...
	cmd := exec.CommandContext(ctx, cfg.remoteFixtureServer, "-rpc")
	...
	rpcCL, err := rpc.NewClient(ctx, stdout, stdin, &protocol.HandshakeRequest{NeedUserServices: false}, func(ctx context.Context) error {
		if err := cmd.Process.Kill(); err != nil {
			return err
		}
		cmd.Wait() // ignore `singal: killed`
		return nil
	})
	if err != nil {
...
func runFixtureAndTests(ctx context.Context, cfg *Config, state *State, rfcl bundle.FixtureService_RunFixtureClient, remoteFixt string, runTests func(ctx context.Context, fixtErr []string) error) (retErr error) {
	...
	if remoteFixt != "" {
		...
		if state.softwareFeatures == nil {
			state.softwareFeatures = &dep.SoftwareFeatures{}
		}
		// push
			...
			Control: &bundle.RunFixtureRequest_Push{
				Push: &bundle.RunFixturePushRequest{
					...
					Config: &bundle.RunFixtureConfig{
						...
						AvailableSoftwareFeatures:   state.softwareFeatures.Available,
						UnavailableSoftwareFeatures: state.softwareFeatures.Unavailable,
	...
	for _, bt := range bundleRemoteFixtTests {
		...
		for _, fixtTests := range bt.tests {
			...
			if err := runFixtureAndTests(ctx, cfg, state, rf.cl, remoteFixt, func(ctx context.Context, setUpErrs []string) error {
				res, err := runLocalTestsSub(ctx, names, remoteFixt, setUpErrs, cfg, state)
...
// It can return partial results and an error when error happens mid-tests.
func runLocalTestsSub(ctx context.Context, names []string, remoteFixt string, setUpErrs []string, cfg *Config, state *State) ([]*EntityResult, error) {
