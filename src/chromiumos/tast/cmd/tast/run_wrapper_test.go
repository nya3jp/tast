...
func (w *stubRunWrapper) run(ctx context.Context, cfg *run.Config, state *run.State) (run.Status, []*run.EntityResult) {
	w.runCtx, w.runCfg, w.runState = ctx, cfg, state
	return w.runStatus, w.runRes
}

func (w *stubRunWrapper) writeResults(ctx context.Context, cfg *run.Config, state *run.State, results []*run.EntityResult, complete bool) error {
