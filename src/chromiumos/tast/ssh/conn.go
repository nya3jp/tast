...
// protocol is passed to net.Listen as the network type. Only TCP protocols are supported.
...
func (s *Conn) ForwardLocalToRemote(protocol, localAddr, remoteAddr string, errFunc func(error)) (*Forwarder, error) {
	...
	l, err := net.Listen(protocol, localAddr)
	if err != nil {
		return nil, err
	}
...
// protocol is passed to net.Dial as the network type. Only TCP protocols are supported.
