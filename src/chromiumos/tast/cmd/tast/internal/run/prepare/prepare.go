...
func getDataFilePaths(ctx context.Context, cfg *config.Config, state *config.State, hst *ssh.Conn) (
	...
	cfg.Logger.Debugf("localFixts = %v", localFixts)

	var entities []*testing.EntityInfo
	for _, t := range ts {
	...
	cfg.Logger.Debugf("entities = %v", entities)

	seenPaths := make(map[string]struct{})
	for _, e := range entities {
		cfg.Logger.Debugf("e.Data = %v, e.Name = %v", e.Data, e.Name)
