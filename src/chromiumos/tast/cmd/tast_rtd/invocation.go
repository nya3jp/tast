...
func invokeTast(logger *log.Logger, inv *rtd.Invocation) error {
	...
	for _, r := range inv.Requests[1:] {
		if r.Environment.WorkDir != "" {
			continue
		}
		if r.Environment.WorkDir == args.resultDir {
			continue
		}
		if err := os.RemoveAll(r.Environment.WorkDir); err != nil {
			return fmt.Errorf("failed to remove working directory %v: %v", r.Environment.WorkDir, err)
		}
		if err := os.Symlink(args.resultDir, r.Environment.WorkDir); err != nil {
			return fmt.Errorf("failed to create symbolic link %v: %v", r.Environment.WorkDir, err)
		}
	}
	// Run tast.
