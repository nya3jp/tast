...
func TestLoadExternalDataMap(t *testing.T) {
	...
	m, err := newExternalDataMap(logging.NewSimple(&bytes.Buffer{}, 0, false), cp)
	if err != nil {
		t.Fatalf("newExternalDataMap(..., %v) failed: %v", cp, err)
	}
...
func TestFetchExternalData(t *testing.T) {
	...
	if err = m.fetchFiles(files, dd); err != nil {
		t.Fatalf("fetchFiles(%v, %q) failed: %v", files, dd, err)
	}
...
func TestLoadBadExternalDataMap(t *testing.T) {
	...
	for _, data := range []string{
		...
		_, err := newExternalDataMap(logging.NewSimple(&bytes.Buffer{}, 0, false), filepath.Join(td, conf))
		if err == nil {
