...
func getBundlesAndTests(args *Args) (bundles []string, tests []*testing.RunnableTest, err *command.StatusError) {
	var glob string
	switch args.Mode {
	case RunTestsMode:
...
func getTests(args *Args, bundles []string) (tests []*testing.RunnableTest,
	...
	for _, b := range bundles {
		...
		go func() {
			...
			ts := make([]*testing.RunnableTest, 0)
	...
	bundleTests := make(map[string][]*testing.RunnableTest)
