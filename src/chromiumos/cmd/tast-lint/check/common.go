...
func getStringLiteralType(s string) (strtype stringLitType, ok bool) {
	if s == "" {
		return 0, false
	}
	quote := s[0]
...
// In auto format system, this function is ideally used after
// the unquoted string does not contain back quote.
// If any error occured in case, this function returns double quoted value
// by default whichever string literal type is.
