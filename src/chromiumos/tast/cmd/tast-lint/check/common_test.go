...
func verifyAutoFix2(t *testing.T, lintfunc func(*token.FileSet, *ast.File, bool) (*token.FileSet, *ast.File, []*Issue), files, expects map[string]string) {
	t.Helper()
	...
	// pretty.Compare(files,)

	for filename := range files {
		if diff := diff.Diff(files[filename], expects[filename]); diff != "" {
			fmt.Println(files[filename], expects[filename])
			t.Errorf("AutoFix failed %s; (-got +want)\n%s", filename, diff)
		}
