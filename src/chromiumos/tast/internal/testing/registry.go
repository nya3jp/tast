...
func (r *Registry) AddFixture(f *Fixture) error {
	if _, ok := r.allFixtures[f.Name]; ok {
		return fmt.Errorf("fixture %s already defined", f.Name)
	}
...
// Fixture returns the matching fixture instance.
func (r *Registry) Fixture(name string) *Fixture {
	return r.allFixtures[name]
}
