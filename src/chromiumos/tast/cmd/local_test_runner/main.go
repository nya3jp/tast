...
func main() {
	...
	cfg := runner.Config{
		...
		SoftwareFeatureDefinitions: map[string]string{
			...
			"dlc":                "dlc && dlc_test",
			"dptf":               "dptf",
			"device_crash":       "!samus", // Samus devices do not reliably come back after kernel crashes. https://crbug.com/1045821
			...
			// asuka, banon, caroline, cave, celes, chell, cyan, edgar, kefka, reks, relm, sentry, terra, ultima, and wizpig have buggy EC firmware and cannot capture crash reports. https://issuetracker.google.com/issues/172228823
			// drallion and sarien have do not support the "crash" EC command. https://crbug.com/1123716
			// guado, tidus, rikku, veyron_fievel, and veyron_tiger do not have EC firmware. https://crbug.com/1123716. TODO(https://crbug.com/1124554) Use an EC hardware dep for these rather than a software dep.
			// nocturne only sporadically captures EC panics. https://crbug.com/1135798
			"ec_crash":                "!(asuka || banon || caroline || cave || celes || chell || cyan || edgar || kefka || reks || relm || sentry || terra || ultima || wizpig || drallion || sarien || guado || tidus || rikku || veyron_fievel || veyron_tiger || nocturne)",
