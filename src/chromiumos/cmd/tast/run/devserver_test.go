...
func TestParseGSURL(t *testing.T) {
	for _, tc := range []struct {
		...
		{"http://chromeos-image-archive/path/to/file.bin", "", "not a gs:// URL"},
		{"gs://secret-bucket/path/to/file.bin", "", "not a whitelisted bucket"},
		{"gs://chromeos-image-archive//path/to/file.bin", "", "not a clean URL"},
		{"gs://chromeos-image-archive/path/to/file.bin/", "", "not a clean URL"},
		{"gs://chromeos-image-archive/../path/to/file.bin", "", "not a clean URL"},
		{"gs://chromeos-image-archive/path/to/../file.bin", "", "not a clean URL"},
		{"#$%", "", "failed to parse URL"},
		...
		if tc.errPrefix == "" {
			...
			if err == nil {
				t.Errorf("parseGSURL(%q) = %q; want error with prefix %q", tc.gsURL, path, tc.errPrefix)
			} else if !strings.HasPrefix(err.Error(), tc.errPrefix) {
				t.Errorf("parseGSURL(%q) returned error %q; want error with prefix %q", tc.gsURL, err.Error(), tc.errPrefix)
			}
		}
	}
}
