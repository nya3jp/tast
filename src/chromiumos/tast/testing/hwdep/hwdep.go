...
func Model(names ...string) Condition {
	...
	return Condition{Satisfied: func(f *dep.HardwareFeatures) error {
		...
		return errors.Errorf("ModelId \"%s\" did not match", modelID)
	}}
}
...
func SkipOnModel(names ...string) Condition {
	...
	return Condition{Satisfied: func(f *dep.HardwareFeatures) error {
		...
		for _, name := range names {
			if name == modelID {
				return errors.Errorf("ModelId \"%s\" matched with skip-on list", modelID)
			}
		}
		return nil
	}}
...
func Platform(names ...string) Condition {
	...
	return Condition{Satisfied: func(f *dep.HardwareFeatures) error {
		...
		return errors.Errorf("PlatformId \"%s\" did not match", platformID)
	}}
}
...
func SkipOnPlatform(names ...string) Condition {
	...
	return Condition{Satisfied: func(f *dep.HardwareFeatures) error {
		...
		for _, name := range names {
			if name == platformID {
				return errors.Errorf("PlatformId \"%s\" matched with skip-on list", platformID)
			}
		}
		return nil
	}}
