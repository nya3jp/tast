...
Tast provides a way to control run/skip tests based on the device characteristic.
...
You can list multiple `Condition`s to provide `hwdep.D`. In the case,
...
[hwdep package]: https://chromium.googlesource.com/chromiumos/platform/tast/+/master/src/chromiumos/tast/testing/hwdeps/

### Adding new hardware condition
...
For example, whether the `hwdep.Touchscreen()` can be checked by
whether `device.Config.hardware_features` contains
`HARDWARE_FEATURE_TOUCHSCREEN`.

Any condition should should not on any other surrounding environment,
...
internally on Tast runtime. In the future, Chrome OS infra will provide
the instance.
