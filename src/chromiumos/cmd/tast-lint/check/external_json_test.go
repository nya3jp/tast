...
`
	...
	expects := []string{
		path + ": include the date as a suffix in the filename like \"gs://chromiumos-test-assets-public/tast/cros/arcapp/ArcWMTestApp_24_YYYYMMDD.apk\"",
	}
...
`
	...
	issues := ExternalJSON(path, []byte(code))
	expects := []string{
		path + ": include the date as a suffix in the filename like \"gs://chromiumos-test-assets-public/tast/cros/arcapp/ArcWMTestApp_24_20191131.invalid_YYYYMMDD.apk\"",
	}
	verifyIssues(t, issues, expects)
}
