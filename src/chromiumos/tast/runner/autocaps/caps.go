...
// State describes the state of a given capability.
type State int
...
// info contains is used to determine which capabilities are set; if nil, system info will be loaded by this function.
...
func Read(dir string, info *SysInfo) (map[string]State, error) {
	...
	for _, fi := range fis {
		if fi.Name() == managedFile || !fileRegexp.MatchString(fi.Name()) {
			continue
		}
