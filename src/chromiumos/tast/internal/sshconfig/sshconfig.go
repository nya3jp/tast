...
func ResolveHost(addr string, configFiles ...string) (resolvedHost string, err error) {
	if len(configFiles) == 0 {
		// Use default config file if they are not supplied.
		configFiles = []string{"~/.ssh/config", "/etc/ssh/ssh_config"}
	}
	...
	for _, f := range configFiles {
		...
		if err := read(f, baseDir, openedFiles, &sc); err != nil {
			return "", err
		}
	}
...
// read reads one or more files that match the argument configFileName.
// The argument configFileName can have wildcards.
// The argument baseDir is used for Include statement withour absolute path.
func read(configFileName, baseDir string, openedFiles map[string]struct{}, sc *block) error {
	fileNames, err := expandFilePath(configFileName, baseDir)
	if err != nil {
		return err
	}
	...
	for scanner.Scan() {
		...
		case strings.EqualFold(strs[0], "Include"):
			for _, f := range strs[1:] {
				if err := read(f, baseDir, openedFiles, curConfig); err != nil {
					return err
				}
			}
...
// expandFilePath expands a path pattern to a list of absolute file names that match the problem.
...
func expandFilePath(f, baseDir string) ([]string, error) {
