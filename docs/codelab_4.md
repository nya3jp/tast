...
host. They are needed when a test needs to reboot during the test
execution for any reason.\
The remote host can communicate with the DUT via ssh or gRPC
services during the test, which are used to execute code on the DUT itself.
...
parts:
  ...
  2. One or more gRPC services which execute the required code on
    the DUT, i.e. for login, the general test logic and cleaning up the DUT.


## GRPC services

The gRPC services are created using [protocol buffers](go/proto-gdh)
to define their methods, their respective request and response message types.\
...
(https://osscs.corp.google.com/chromiumos/chromiumos/codesearch/+/main:src/platform/tast-tests/src/chromiumos/tast/local/bundles/cros/policy/system_timezone_service.go)

## Remote test

The remote tests themselves are put into their respective folder in
`tast-tests/local_tests`, so in the case of a policy test they are placed in
`tast-tests/local_tests/policy`.\
...
(https://osscs.corp.google.com/chromiumos/chromiumos/codesearch/+/main:src/platform/tast-tests/src/chromiumos/tast/remote/bundles/cros/policy/system_timezone.go).
