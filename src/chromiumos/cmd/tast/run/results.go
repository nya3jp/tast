...
// diagnoseFunc is called after a run error is encountered while reading test results to get additional
// information about the cause of the error. An empty string should be returned if additional information
// is unavailable.
type diagnoseRunErrorFunc func() string
...
func newResultsHandler(ctx context.Context, cfg *Config, crf copyAndRemoveFunc, df diagnoseRunErrorFunc) (*resultsHandler, error) {
	r := &resultsHandler{
		ctx:       ctx,
		cfg:       cfg,
...
func (r *resultsHandler) processMessages(mch <-chan interface{}, ech <-chan error) ([]TestResult, error) {
...
func readTestOutput(ctx context.Context, cfg *Config, r io.Reader, crf copyAndRemoveFunc, df diagnoseRunErrorFunc) (
	[]TestResult, error) {
	rh, err := newResultsHandler(ctx, cfg, crf, df)
	if err != nil {
		return nil, err
	}
	defer rh.close()
	...
	return rh.processMessages(mch, ech)
}
