...
func diagnoseReboot(ctx context.Context, cfg *Config, outDir string) string {
	...
	if err != nil {
		cfg.Logger.Log("Failed to read unified system logs: ", err)
		out, err = cfg.hst.Command("journalctl", "--source=journal", "--quiet", "--boot="+denseBootID, "--lines=1000").Output(ctx)
		if err != nil {
			cfg.Logger.Log("Failed to read journal logs: ", err)
		}
	}
	logs := string(out)
