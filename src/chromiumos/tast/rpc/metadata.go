...
// outgoingMetadata extracts EntityContext from ctx and converts it to metadata.MD.
// It is called on gRPC clients to forward EntityContext over gRPC.
...
// incomingEntityContext creates EntityContext from metadata.MD.
// It is called on gRPC servers to forward EntityContext over gRPC.
func incomingEntityContext(md metadata.MD) *testing.EntityContext {
	softwareDeps := md[metadataSoftwareDeps]
	return &testing.EntityContext{
