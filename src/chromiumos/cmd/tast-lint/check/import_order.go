...
func ImportOrder(path string, in []byte) []*Issue {
	...
	if !bytes.Equal(in, out) {
		...
		return []*Issue{&Issue{
			Pos: token.Position{Filename: path},
			Msg: fmt.Sprintf("Import should be grouped by empty line and sorted in the following order: stdlib, third_party lib, chromiumos: %s", detail),
		}}
	}
...
// takeDiff takes the diff between orig and expect, and returns the diff in
// unified diff format.
func takeDiff(orig []byte, expect []byte) (string, error) {
	f1, err := writeTempFile(orig, "orig")
	if err != nil {
		return "", err
	}
	defer os.Remove(f1)

	f2, err := writeTempFile(expect, "expect")
	if err != nil {
		return "", err
	}
	defer os.Remove(f2)
