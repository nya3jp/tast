...
func runDownloads(ctx context.Context, dataDir string, jobs []*downloadJob, cl devserver.Client, lf func(msg string)) {
	...
	for i := 0; i < parallelism; i++ {
		go func() {
			for job := range jobCh {
				lf(fmt.Sprintf("Downloading %s", job.link.computedURL))
				start := time.Now()
		...
		case res := <-resCh:
			if res.err != nil {
				...
				lf(fmt.Sprintf("Finished downloading %s (%d bytes, %v, %.1fMB/s)",
					res.job.link.computedURL, res.job.link.Size, res.duration.Round(time.Millisecond), mbs))
			}
			finished++
...
func runDownload(ctx context.Context, dataDir string, job *downloadJob, cl devserver.Client) error {
	...
	if _, err := cl.DownloadGS(ctx, f, job.link.computedURL); err != nil {
		return err
	}
