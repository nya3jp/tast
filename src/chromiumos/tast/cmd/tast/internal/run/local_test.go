...
func TestLocalList(t *gotesting.T) {
	...
	listed := make([]testing.RunnableTest, len(results))
	for i := 0; i < len(results); i++ {
		listed[i] = results[i].RunnableTest
	}
...
func TestLocalContinueAfterFailure(t *gotesting.T) {
	...
	td.runFunc = func(args *runner.Args, stdout, stderr io.Writer) (status int) {
		...
		case 1:
			...
			mw.WriteMessage(&control.TestStart{Time: time.Unix(2, 0), Test: testing.RunnableTest{Name: test1}})
			return 1
		case 2:
			...
			mw.WriteMessage(&control.TestStart{Time: time.Unix(4, 0), Test: testing.RunnableTest{Name: test2}})
			mw.WriteMessage(&control.TestEnd{Time: time.Unix(5, 0), Name: test2})
			mw.WriteMessage(&control.TestStart{Time: time.Unix(6, 0), Test: testing.RunnableTest{Name: test3}})
			return 1
		default:
