...
func main() {
	...
	const (
		...
		crostiniUnstableBoards = `auron_paine || auron_yuna || banon || bob || buddy || celes || coral || cyan || edgar || elm || fizz || gandof || grunt || hana || kefka || kevin64 || kevin || kukui || kefka || lulu || nocturne || octopus || reks || relm || samus || sarien || setzer || terra || ultima || wizpig`
	)
	cfg := runner.Config{
		...
		SoftwareFeatureDefinitions: map[string]string{
			...
			"plugin_vm":              "pita && eve",           // boards that can run Plugin VM, and are licensed to do so in the lab environment.
