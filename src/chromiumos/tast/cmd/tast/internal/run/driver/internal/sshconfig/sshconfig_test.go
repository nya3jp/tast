...
func testForResolveHostFromDefaultConfig(t *testing.T, input, expectedHost, expectedProxyCommand string) {
	...
	testResolveHostFromFiles(t, input, expectedHost, expectedProxyCommand, fileParams)
}
...
func TestResolveHostFromFilesIncludeTop(t *testing.T) {
	testForResolveHostFromDefaultConfig(t, "octopus", "127.0.0.1:2222", "proxy_command octopus")
}
...
func TestResolveHostFromFilesWithPortOverride(t *testing.T) {
	testForResolveHostFromDefaultConfig(t, "octopus:1", "127.0.0.1:1", "proxy_command octopus")
}
...
func TestResolveHostFromFilesWithLoop(t *testing.T) {
	...
	if _, _, err := sshconfig.ResolveHostFromFiles("hana", fileParams); err == nil {
...
func testResolveHostFromFiles(t *testing.T, input, expectedHost, expectedProxyCommand string, fileParams []sshconfig.FileParam) {
	resolvedHost, resolvedProxyCommand, err := sshconfig.ResolveHostFromFiles(input, fileParams)
	if err != nil {
		t.Fatalf("Encounter error while calling ResolveHostFromFiles(%q, %q): %v", input, fileParams, err)
	}
	t.Logf("ResolveHostFromFiles(%q) = (%q, %q)", input, resolvedHost, resolvedProxyCommand)
	if resolvedHost != expectedHost || resolvedProxyCommand != expectedProxyCommand {
		t.Fatalf("ResolveHostFromFiles(%q, %q) = (%q, %q); want (%q, %q)", input, fileParams, resolvedHost, resolvedProxyCommand, expectedHost, expectedProxyCommand)
	}
}
