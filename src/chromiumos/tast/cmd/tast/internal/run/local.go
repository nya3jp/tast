...
func runLocalTests(ctx context.Context, cfg *config.Config, state *config.State) (res []*jsonprotocol.EntityResult, retErr error) {
	...
	for _, bt := range bundleRemoteFixtTests {
		...
		// If all the tests depend on remote fixtures, run empty test set with
		// no remote fixture.
		// Local bundles run its ready hook (DUT clean up) only when no remote
		// fixutres are used. Here we make sure the ready hook runs even if all
		// the tests depend on remote fixtures.
