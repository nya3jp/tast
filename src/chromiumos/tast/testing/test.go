...
type Param struct {
	// Name is the name of this parametric test. Full name of the test
	// case will be category.TestFuncName.param_name.
	// It is expected to be matched with [a-z0-9_]*. Note that unnamed
	// param case is allowed. In the case, the full test case name for the
	// param will be category.TestFuncName, which is not followed by '.'.
	Name string
	...
	// Val is the value which can be retrieved from testing.State.Param()
	// method.
	Val interface{}
}
...
// paramNameRegexp validates test names, which should consist of a package name,
// a period, and the name of the exported test function.
