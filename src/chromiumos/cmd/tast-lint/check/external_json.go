...
func ExternalJSON(path string, in []byte) []*Issue {
	...
	url := dataFile.URL
	if url == "" ||
		(!strings.HasPrefix(url, "gs://chromiumos-test-assets-public/tast/") && !strings.HasPrefix(url, "gs://chromeos-test-assets-private/tast/")) {
		return nil
	}
	...
	return []*Issue{{
		...
		Msg:  fmt.Sprintf("include the date as a suffix in the filename like \".../%s%s\"", strings.TrimRight(base, filepath.Ext(url))+"_YYYYMMDD", filepath.Ext(url)),
