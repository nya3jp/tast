...
// RunOption is options which can be passed to Run, Output,
...
//
// Usage:
//   cmd.Run(..., ssh.DumpLogOnError(s.Logger))
...
func (c *Cmd) setupSession(sess *ssh.Session) error {
	...
	// Unlike Cmd in os/exec, x/crypto/ssh isn't concurrent safe if Stderr
	// and Stdout are the same writer.
...
// session.
func (c *Cmd) waitAndClose(ctx context.Context, f func() error, opts ...*RunOption) error {
	if c.state != stateStarted {
