...
To examine sentinel errors which may be `Wrap`ed, use [errors.Is] or [errors.As].
The usage is parallel to the same name functions in the official errors package.
...
It is recommended to wrap when you cross package boundary, because everything
beneath them are implementation details. Otherwise it is fine to return an
error without wrapping, if you can't really add much context to make debugging
easier. Use your best judge to decide wrap or not.

Following quotes from
*[The Go programming language] 5.4.1 Error-Handling Strategies*
will be useful to design good errors.
