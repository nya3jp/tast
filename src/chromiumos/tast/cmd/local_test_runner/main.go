...
func main() {
	...
	cfg := runner.Config{
		...
		SoftwareFeatureDefinitions: map[string]string{
			...
			// TODO(crbug.com/1070299): Remove the below hard-coded devices and use Intel WiFi
			// dependency when wifi hardware dependencies are implemented.

			// TODO(crbug.com/1115620): remove "Elm" and "Hana" after unibuild migration completed.
			// Also, volteer is normally using Intel WiFi (HrP2), but the devices in the lab are
			// incorrectly equipped with Realtek RTL8822 chips. This test should skip volteer devices
			// until this is fixed (see b:171754540).
			// The list of boards with Intel WiFi chips is long, so instead of listing all the boards that have Intel WiFi chips, skip the ones that don't have it.
			"intel_wifi_chip": "!(bob || elm || grunt || hana || jacuzzi || kevin || kukui || oak || scarlet || trogdor || volteer || veyron_fievel || veyron_mickey || veyron_tiger)",
		},
