...
var varFileBaseNameRE = regexp.MustCompile(`^([a-zA-Z]+(?:\.[a-zA-Z]+)?\.)yaml$`)

// varLastPartRE matches with a valid last part (after the last dot) of variable names.
var varLastPartRE = regexp.MustCompile(`^[a-zA-Z][0-9A-Za-z_]*$`)
...
func SecretVarFile(path string, data []byte) (issues []*Issue) {
	...
	if m := varFileBaseNameRE.FindStringSubmatch(base); m == nil {
		addIssue(fmt.Sprintf("File's basename doesn't match expected regex %s", varFileBaseNameRE))
	} else {
		wantPrefix = m[1]
	}
	...
	for varName := range vars {
		...
		case !varLastPartRE.MatchString(strings.TrimPrefix(varName, wantPrefix)):
			addIssue(fmt.Sprintf("var name %s violates naming convention; it should match %s[a-zA-Z][0-9A-Za-z_]*", varName, wantPrefix))
		}
	}
	return
}
