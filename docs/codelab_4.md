...
The remote host can communicate with the DUT via ssh or [gRPC](go/grpc)
...
parts:
  ...
  2. One ore more [gRPC](go/grpc) services which execute the required code on
    the DUT, i.e. for login, the general test logic and cleaning up the DUT.


## GRPC services

The [gRPC](go/grpc) services are created using [protocol buffers](go/proto-gdh)
...
NOTE: When commiting a remote test with gRPC services make sure to also submit the
generated files.
