...
if err := conn.Eval(ctx, `return new Promise((resolve, reject) => {
		...
		if element.innerText !== "some text" {
			return reject(throw new Error("Unexpected inner text: want some text; got " + element.innerText));
		}
		return resolve(true)
	})`, &foundIt); err != nil {
	// Do error handling here.
...
```
The [chrome.Eval()] can also be used on tast's background page which gives access to other APIs. A connection to the background page can be created with [chrome.TestAPIConn()].
...
[testing.PollOptions](https://godoc.org/chromium.googlesource.com/chromiumos/platform/tast.git/src/chromiumos/tast/testing#PollOptions)
