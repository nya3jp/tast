...
func (lc *listCmd) Execute(ctx context.Context, f *flag.FlagSet, _ ...interface{}) subcommands.ExitStatus {
	...
	if len(lc.inputFiles) > 0 {
		...
		tests = make([]*testing.RunnableTest, len(results))
		for i := range results {
			tests[i] = &results[i].RunnableTest
		}
	}
...
func (lc *listCmd) printTests(tests []*testing.RunnableTest) error {
	if lc.json {
...
func importTests(p string, reg *testing.Registry) error {
	...
	var ts []*testing.RunnableTest
	if err := json.NewDecoder(f).Decode(&ts); err != nil {
		return err
	}
	for _, t := range ts {
		if err := reg.AddRunnableTest(t); err != nil {
