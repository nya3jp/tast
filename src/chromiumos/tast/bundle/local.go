...
type LocalDelegate struct {
	...
	// Faillog is called for each test case is failed. It is expected to be save debugging info.
...
func Local(clArgs []string, stdin io.Reader, stdout, stderr io.Writer, delegate *LocalDelegate) int {
	...
	cfg := runConfig{
		postTestFunc: func(ctx context.Context, s *testing.State) {
			...
			if delegate != nil && delegate.Faillog != nil {
	...
	if delegate != nil && delegate.Ready != nil {
