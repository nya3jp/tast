...
func runTests(ctx context.Context, cfg *config.Config, state *config.State, cc *target.ConnCache) (results []*resultsjson.Result, retErr error) {
	...
	if deadline, ok := ctx.Deadline(); ok {
		postReserve := time.Until(deadline) / 2
		if postReserve > maxPostReserve {
			postReserve = maxPostReserve
		}
		runDeadline := time.Now().Add(postReserve)
		var cancel xcontext.CancelFunc
		ctx, cancel = xcontext.WithDeadline(ctx, runDeadline, errors.Errorf("%v: global timeout reached", context.DeadlineExceeded))
