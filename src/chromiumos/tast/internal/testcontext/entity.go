...
// currentEntityKeyType is the type of the key used for attaching a
// CurrentEntity to a context.Context.
type currentEntityKeyType struct{}
...
func WithCurrentEntity(ctx context.Context, ec *CurrentEntity) context.Context {
	return context.WithValue(ctx, currentEntityKeyType{}, ec)
}
...
func OutDir(ctx context.Context) (dir string, ok bool) {
	ec, ok := ctx.Value(currentEntityKeyType{}).(*CurrentEntity)
	if !ok || ec.OutDir == "" {
		return "", false
	}
