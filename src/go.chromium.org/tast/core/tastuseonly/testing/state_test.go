...
func TestReportError(t *gotesting.T) {
	...
	for _, e := range []*protocol.Error{e0, e1} {
		...
		if exp := fmt.Sprintf("\tat go.chromium.org/tast/core/tastuseonly/testing_test.TestReportError (%s:%d)", filepath.Base(e.GetLocation().GetFile()), e.GetLocation().GetLine()); lines[1] != exp {
...
func TestReportErrorInPrecondition(t *gotesting.T) {
	...
	for _, e := range []*protocol.Error{e0, e1} {
		...
		if exp := fmt.Sprintf("\tat go.chromium.org/tast/core/tastuseonly/testing_test.TestReportErrorInPrecondition (%s:%d)", filepath.Base(e.GetLocation().GetFile()), e.GetLocation().GetLine()); lines[1] != exp {
			t.Errorf("Second line of stack trace %q doesn't match %q", string(e.GetLocation().GetStack()), exp)
		}
	}
}
...
func TestExtractErrorSimple(t *gotesting.T) {
	...
	if exp := "meow\n\tat go.chromium.org/tast/core/tastuseonly/testing_test.TestExtractErrorSimple"; !strings.HasPrefix(e.GetLocation().GetStack(), exp) {
		t.Errorf("Stack trace %q doesn't start with %q", e.GetLocation().GetStack(), exp)
	}
	if exp := "meow\n\tat go.chromium.org/tast/core/tastuseonly/testing_test.errorFunc"; !strings.Contains(e.GetLocation().GetStack(), exp) {
...
func TestExtractErrorHeuristic(t *gotesting.T) {
	...
	for _, e := range out.Data.Errs {
		...
		if exp := "Failed something\n\tat go.chromium.org/tast/core/tastuseonly/testing_test.TestExtractErrorHeuristic"; !strings.HasPrefix(e.GetLocation().GetStack(), exp) {
			t.Errorf("Stack trace %q doesn't start with %q", e.GetLocation().GetStack(), exp)
		}
		if exp := "\nmeow\n\tat go.chromium.org/tast/core/tastuseonly/testing_test.errorFunc"; !strings.Contains(e.GetLocation().GetStack(), exp) {
			t.Errorf("Stack trace %q doesn't contain %q", e.GetLocation().GetStack(), exp)
