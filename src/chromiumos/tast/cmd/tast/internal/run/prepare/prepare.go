...
func allFixturesNeeded(fixtures []*jsonprotocol.EntityInfo, tests []*jsonprotocol.EntityWithRunnabilityInfo) []*jsonprotocol.EntityInfo {
...
func getDataFilePaths(ctx context.Context, cfg *config.Config, state *config.State, hst *ssh.Conn) (
	...
	if fixts, ok := localFixts[filepath.Join(cfg.LocalBundleDir, cfg.BuildBundle)]; ok {
		entities = append(entities, allFixturesNeeded(fixts, ts)...)
	}
