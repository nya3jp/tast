...
Finally, we need to read the output from `dmesg`. This is fairly tricky:
...
read everything that we're trying to write to the channel. To handle this case,
we put the write into a `select` statement with a `default` case that will be
executed immediately if the channel isn't currently writable. If we hit that, we
sleep briefly (to avoid burning CPU cycles unnecessarily), check whether the
context's deadline has been reached (indicating that the test completed), and
then try to write to the channel again. We declare a `WriteLoop` [label] to let
us break out of the outer loop when the write is successful.
...
If we see the expected message, we break out of the loop.
