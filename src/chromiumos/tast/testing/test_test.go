...
func TestAssignName(t *gotesting.T) {
	...
	if exp := "testing.Func1"; test.Name != exp {
...
func TestCheckFuncName(t *gotesting.T) {
	test := Test{Func: TESTTEST}
	if err := test.finalize(true); err != nil {
		t.Error("Got error when finalizing test with valid test function name: ", err)
	} else if exp := "testing.TESTTEST"; test.Name != exp {
		t.Errorf("Test was given name %q; want %q", test.Name, exp)
	}
}

func TestCheckFuncNameInvalid(t *gotesting.T) {
	test := Test{Func: Func1}
	if err := test.finalize(true); err == nil {
		t.Error("Didn't get expected error when finalizing test with inlavid test function name")
	}
}
