...
const (
	noTopAddTestMsg     = `testing.AddTest() should be called only at a top statement of init()`
...
func verifyInit(fs *token.FileSet, node ast.Decl) []*Issue {
	...
	if !ok || decl.Name.Name != "init" {
		// Not a init() function declaration. Skip.
		return nil
	}
...
func verifyInitBody(fs *token.FileSet, stmt ast.Stmt) []*Issue {
	...
	// The compiler should already check the type. Skip it.
	var issues []*Issue
	hasContacts := false
...
// - an ast.Ident node
// - a ast.SelectorExpr node whose .X node is convertible by toQualifiedName.
