...
func TestRunPrecondition(t *gotesting.T) {
	...
	// The test should be able to access the data via State.Pre.
	test := &Test{
		...
		Func: func(ctx context.Context, s *State) {
			if s.Pre() == nil {
				s.Fatal("Precondition data not supplied")
			} else if d, ok := s.Pre().(*data); !ok {
				s.Fatal("Precondition data didn't have expected type")
			} else if d != preData {
				s.Fatalf("Got precondition data %v; want %v", d, preData)
			}
		},
		Timeout: time.Minute,
	}
