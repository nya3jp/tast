	...
	if remoteFixt != "" {
		handleResponses := func() (fixtErrs []*bundle.RunFixtureError, retErr error) {
			...
			for {
				...
				cfg.Logger.Logf("got rfcl.Recv() = %v %v", msg, err)
		...
		if len(fixtErrs) > 0 {
			pushErrs = append(pushErrs, fmt.Sprintf("fixture %s has failed: %v; see %v for details", remoteFixt, fixtErrs[0].Reason, fixtLogFile))
		}
