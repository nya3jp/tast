...
// request, prepare necessary dependencies, and call into entity entry points
// properly.
//
// Entity entry point functions usually accept two arguments: context.Context
// and testing.*State (an exact State type varies by the entity type). A State
// type provides API methods that interact with the framework. Though the type
// is named "State" for historical reason, it is not something that keeps track
// of the state of the current entity execution as you may imagine. If we were
// able to rename them, we might want to name them as "TestAPI" or similar.
// But now that we've got a lot of tests, it seems not worth doing now.
//
...
// allocated. So it might be easier to understand Root objects as "real state
// objects". Root objects are private to the framework. User code always access
// Root objects indirectly via State objects.
//
