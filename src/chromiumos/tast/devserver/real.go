...
// SetServers checks if devservers at dsURLs are up, and selects a subset of devservers to use.
...
// set ctx's timeout carefully since this method can block until it expires if any devserver is down.
// This method must be called exactly once after construction.
func (c *RealClient) SetServers(ctx context.Context, dsURLs []string) {
	...
	sort.Slice(srvs, func(i, j int) bool {
		return srvs[i].url < srvs[j].url
	})

	c.servers = srvs
}

// upServerURLs returns URLs of devservers up running.
...
func (c *RealClient) findStaged(ctx context.Context, bucket, path string) (dsURL string, err error) {
	ch := make(chan string, len(c.upServerURLs()))

	for _, dsURL := range c.upServerURLs() {
		go func(dsURL string) {
...
func (c *RealClient) checkStaged(ctx context.Context, dsURL, bucket, path string) error {
	staticURL := fmt.Sprintf("%s/static/%s", dsURL, url.PathEscape(path))
...
func (c *RealClient) chooseServer(gsURL string) string {
	dsURLs := make([]string, len(c.upServerURLs()))
	copy(dsURLs, c.upServerURLs())
