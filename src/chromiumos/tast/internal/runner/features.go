...
func newDeviceConfigAndHardwareFeatures() (dc *protocol.DeprecatedDeviceConfig, retFeatures *configpb.HardwareFeatures, warns []string) {
	...
	features.Memory.Profile.SizeMegabytes = int32(memoryBytes / 1_000_000)
...
// findMemorySize returns the total memory size from the content of /proc/meminfo in bytes.
func findMemorySize(output []byte) (int64, error) {
	...
	for sc.Scan() {
		...
		if len(cap) != 2 {
			return 0, fmt.Errorf("unexpected line format: input=%s", tokens[1])
		}
		if cap[1] != "kB" {
			return 0, fmt.Errorf("unexpected unit: got %s, want kB", cap[1])
		}
