...
func writeGetDUTInfoResult(w io.Writer, avail, unavail []string, dc *device.Config, osVersion string) error {
...
func checkRunnerTestDepsArgs(t *testing.T, cfg *Config, checkDeps bool,
	avail, unavail []string, dc *device.Config, osVersion string) {
	t.Helper()
	args := runner.Args{
...
func TestGetDUTInfo(t *testing.T) {
	...
	td.runFunc = func(args *runner.Args, stdout, stderr io.Writer) (status int) {
		...
		writeGetDUTInfoResult(stdout, avail, unavail, dc, osVersion)
		return 0
	}
	...
	checkRunnerTestDepsArgs(t, &td.cfg, true, avail, unavail, dc, osVersion)
