...
func TestRunTestsGetDUTInfo(t *gotesting.T) {
	...
	chromeOSVersion := "octopus-release/R86-13312.0.2020_07_02_1108"
	...
	expectedOSVersion := "Chrome OS Version: " + chromeOSVersion
	if !strings.Contains(td.logbuf.String(), expectedOSVersion) {
		t.Errorf("Cannot find '%v' in log buffer %v", expectedOSVersion, td.logbuf.String())
	}
	if !called {
