...
func TestLogging(t *testing.T) {
	...
	ctx := logging.AttachLogger(context.Background(), logger, true)
	logging.Info(ctx, "aaa")
	logging.Infof(ctx, "b%sb", "b")
	logging.Debug(ctx, "ccc")
...
func TestLogging_Propagate(t *testing.T) {
	...
	ctx := context.Background()
	ctx = logging.AttachLogger(ctx, logger1, true)
	ctx = logging.AttachLogger(ctx, logger2, true)
	ctx = logging.AttachLogger(ctx, logger3, false) // turn off propagation
	ctx = logging.AttachLogger(ctx, logger4, true)
...
func TestLogging_Branch(t *testing.T) {
	...
	ctx1 := logging.AttachLogger(context.Background(), logger1, true)
	ctx2 := logging.AttachLogger(ctx1, logger2, true)
	ctx3 := logging.AttachLogger(ctx1, logger3, true)
...
func TestHasLogger(t *testing.T) {
	...
	ctx = logging.AttachLogger(ctx, logging.NewMultiLogger(), true)
	if !logging.HasLogger(ctx) {
