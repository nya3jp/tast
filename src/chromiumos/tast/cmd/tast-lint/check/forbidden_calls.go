...
func ForbiddenCalls(fs *token.FileSet, f *ast.File, fix bool) []*Issue {
	...
	hasImport, hasError := requireImportErrors(f)
	...
	if len(importsRequired) > 0 || (fix && hasError == nil) {
...
func hasIdentErrors(f *ast.File) bool {
	...
	ast.Inspect(f, func(node ast.Node) bool {
		...
		case *ast.SelectorExpr:
			// Removes the possibility of <exp>.<sel> with something like errors.New, errors.Wrap etc. Also we won't explore the subtree.
			return false
		case *ast.Ident:
