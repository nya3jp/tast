...
func SetUp(t *gotesting.T, opts ...EnvOption) *Env {
	...
	for _, dir := range []string{
		tempDir,
		localBundleDir(rootDir),
		remoteBundleDir(rootDir),
	} {
	...
	fakebundle.InstallAt(t, localBundleDir(rootDir), cfg.LocalBundles)
	fakebundle.InstallAt(t, remoteBundleDir(rootDir), cfg.RemoteBundles)

	// Create a fake local test runner.
	localTestRunner := fakerunner.New(&fakerunner.Config{
		BundleDir: localBundleDir(rootDir),
	...
	remoteTestRunner := fakerunner.New(&fakerunner.Config{
		BundleDir: remoteBundleDir(rootDir),
		StaticConfig: &runner.StaticConfig{
			Type: runner.RemoteRunner,
		},
		OnRunTestsInit: cfg.OnRunRemoteTestsInit,
	})
	lo, err := remoteTestRunner.Install(remoteTestRunnerPath(rootDir))
	if err != nil {
		t.Fatal(err)
	}
	...
	handlers = append(handlers, localTestRunner.SSHHandlers(localTestRunnerPath(rootDir))...)
...
func (e *Env) Config() *config.Config {
	...
	return &config.Config{
		...
		ResDir:              filepath.Join(e.rootDir, "tast/results/latest"),
		TastDir:             filepath.Join(e.rootDir, "tast"),
		TrunkDir:            filepath.Join(e.rootDir, "trunk"),
		LocalRunner:         localTestRunnerPath(e.rootDir),
		LocalBundleDir:      localBundleDir(e.rootDir),
		LocalOutDir:         filepath.Join(e.rootDir, fmt.Sprintf("tmp/out/local.%d", randomID)),
		RemoteRunner:        remoteTestRunnerPath(e.rootDir),
		RemoteBundleDir:     remoteBundleDir(e.rootDir),
		RemoteOutDir:        filepath.Join(e.rootDir, fmt.Sprintf("tmp/out/remote.%d", randomID)),
		RemoteFixtureServer: filepath.Join(remoteBundleDir(e.rootDir), "bundle"),
	}
}
...
func localTestRunnerPath(rootDir string) string {
	return filepath.Join(rootDir, "mock_local_test_runner")
}
func remoteTestRunnerPath(rootDir string) string {
	return filepath.Join(rootDir, "mock_remote_test_runner")
}
func localBundleDir(rootDir string) string {
	return filepath.Join(rootDir, "bundles/local")
}
func remoteBundleDir(rootDir string) string {
	return filepath.Join(rootDir, "bundles/remote")
}
