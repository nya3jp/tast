...
`Desc` is a short, human-readable phrase describing the test, and `Attr`
contains free-form strings naming this test's [attributes]. `informational`
indicates that this test is non-critical, i.e. it won't run on the Chrome OS
Commit Queue or on the Pre-Flight Queue (PFQ) builders that are used to
integrate new versions of Chrome or Android into the OS. All [new tests]
...
After closing our loop and the test function, we're done! If the test reports
one or more errors in the loop, it fails. If no errors have been reported by the
time that the test function returns, then the test passes.
