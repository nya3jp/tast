...
// Package testing provides facilities to run a single entity.
//
// States, roots and mix-ins
//
// The most popular types in this package are State types (State, PreState, ...)
// which provide framework APIs to entities. For an entity, multiple State
// objects can be created (e.g. State.Run creates a new State for a subtest).
//
// To maintain information and "states" common to multiple State objects for an
// entity, a single EntityRoot object (and additionally a TestRoot object in the
// case of a test) is allocated. All State object for an entity must share the
// same instance of a Root object. While Root types are exported, they must be
// kept private to the framework.
//
// Since many methods are common to multiple State types, to reduce code
// duplication, State types typically embed mix-in types which implement common
// methods.
package testing
...
// EntityRoot holds basic immutable information such as TestConfig, and also
// maintains states common to all State objects for an entity, e.g. whether any
// error has been reported.
// All State instances for an entity must share the same instance of EntityRoot.
// EntityRoot is exported but must be kept private to the framework.
...
// TestRoot maintains states common to all State objects for a test, e.g.
// a precondition value.
// All test-related State instances for a test must share the same instance of
// TestRoot.
// TestRoot is exported but must be kept private to the framework.
...
// testMixin implements common methods for test-related State types.
...
func (d *dataFS) Open(name string) (http.File, error) {
	...
	if _, err := os.Stat(filepath.Join((*testMixin)(d).testRoot.entityRoot.cfg.DataDir, name)); os.IsNotExist(err) {
		return nil, errors.New("not found")
	}

	return os.Open((*testMixin)(d).DataPath(name))
}
