...
func runFirstTest(ctx context.Context, patterns []string) (results []TestResult, unstarted []string, err error) {
	if len(patterns) == 0 {
		return nil, nil, nil
	}
	return []TestResult{{}}, patterns[1:], errors.New("failure")
}

// runFirstTestNoUnstarted is runTestsFunc that pretends to run the first test only. It returns nil as unstarted.
func runFirstTestNoUnstarted(ctx context.Context, patterns []string) (results []TestResult, unstarted []string, err error) {
	if len(patterns) == 0 {
		return nil, nil, nil
	}
	return []TestResult{{}}, nil, errors.New("failure")
}

// runNoTestWithUnstarted is runTestsFunc that pretends to run no test. It returns patterns as unstarted as-is.
func runNoTestWithUnstarted(ctx context.Context, patterns []string) (results []TestResult, unstarted []string, err error) {
	if len(patterns) == 0 {
		return nil, nil, nil
	}
