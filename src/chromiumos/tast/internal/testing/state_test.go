...
func TestContactVar(t *gotesting.T) {
	const (
		validName  = "valid"  // registered by test and provided
		unsetName  = "unset"  // registered by test but not provided at runtime
		unregName  = "unreg"  // not registered by test but provided at runtime
	...
	test := &TestInstance{Vars: []string{validName, unsetName}}
	cfg := &RuntimeConfig{Vars: map[string]string{validName: validValue, unregName: unregValue, globalName: globalValue}}
	var out outputSink
	root := NewTestEntityRoot(test, cfg, &out)
	s := root.NewTestState()
	ctx := NewContextWithVars(context.Background(), s, &EntityConstraints{allVars: []string{globalName}})
