...
// isTastFile checks if a file path is under Tast test directories.
func isTastFile(path string) bool {
	path, err := filepath.Abs(path)
	if err != nil {
		return false
	}
	return strings.Contains(path, "src/chromiumos/tast/")
}

// isTastTestFile checks if a file path is under Tast test directories.
func isTastTestFile(path string) bool {
	path, err := filepath.Abs(path)
	if err != nil {
		return false
	}
...
func checkAll(git *git, paths []string, debug bool) ([]*check.Issue, error) {
	...
	for _, path := range paths {
		if !isGoFile(path) || !isTastFile(path) {
			continue
		}
