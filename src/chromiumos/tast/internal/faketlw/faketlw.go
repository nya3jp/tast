...
func (s OperationsServer) GetOperation(ctx context.Context, req *longrunning.GetOperationRequest) (*longrunning.Operation, error) {
	...
	if !ok {
		// TODO: check what Status error should be returned
		// the returned error is compatible with the status package.
		return nil, fmt.Errorf("operation name %s not found", name)
	}
