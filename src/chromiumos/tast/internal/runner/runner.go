...
func Run(clArgs []string, stdin io.Reader, stdout, stderr io.Writer, args *Args, cfg *Config) int {
	...
	case ListFixturesMode:
		if args.ListFixtures == nil {
			return command.WriteError(stderr, command.NewStatusErrorf(statusBadArgs, "args.ListFixtures = nil on ListFixturesMode"))
		}
		fixts, err := fixtures(args.ListFixtures.BundleGlob)
		if err != nil {
			return command.WriteError(stderr, err)
		}
