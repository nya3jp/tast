...
type externalDataMap struct {
	lg       logging.Logger
	fileURLs map[string]string // keys are data file paths relative to bundle dir; values are URLs
}

// newExternalDataMap loads an external_data.conf file at cfgPath.
func newExternalDataMap(lg logging.Logger, cfgPath string) (*externalDataMap, error) {
	fileURLs, err := readExternalDataConfig(cfgPath)
	if err != nil {
		return nil, fmt.Errorf("failed to read %v: %v", cfgPath, err)
	}
	return &externalDataMap{lg, fileURLs}, nil
}
...
func (e *externalDataMap) fetchFiles(files []string, extDataDir string) error {
	for _, p := range files {
		u, ok := e.fileURLs[p]
		if !ok {
			continue
