...
func TestParseBundlePackage(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			"chromiumos/tast/remote/bundles/cros/foo/bar",
			resp{"chromiumos/tast/remote/bundles/cros", "foo", true},
		},
		{
...
`,
			...
			[]string{
				...
				"src/chromiumos/tast/local/bundles/cros/foo/testfile.go:7:2: import of chromiumos/tast/remote/bundles/cros/foo/baz is only allowed from chromiumos/tast/remote/bundles/cros/foo or its descendant",
			},
		},
		{
			`package main
