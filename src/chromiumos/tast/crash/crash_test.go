...
func TestGetCrashes(t *testing.T) {
	...
	gpu := writeCrashFile(t, td, "i915_error_state.log.xz", "")
	...
	sort.Strings(files)
	if exp := []string{barDmp.abs, fooCore.abs, fooDmp.abs, fooLog.abs, fooMeta.abs, fooCompressedLog.abs, gpu.abs}; !reflect.DeepEqual(files, exp) {
