...
func (l *Log) Write(w io.Writer) error {
	...
	for i, s := range l.stages {
		...
		if err := s.write(w, indent, " ", i == len(l.stages)-1); err != nil {
			return err
		}
	}
...
func (s *Stage) write(w io.Writer, initialIndent, followIndent string, last bool) error {
	var err error
	write := func(s string) {
		if err == nil {
	...
	write(fmt.Sprintf("%s[%0.3f, %s", initialIndent, s.Elapsed().Seconds(), mn))
