...
func findSOC() (device.Config_SOCVendor, device.Config_SOC, error) {
	reportErr := func(err error) (device.Config_SOCVendor, device.Config_SOC, error) {
	...
	if err := json.Unmarshal(b, &parsed); err != nil {
		return reportErr(errors.Wrap(err, "failed to parse lscpu result"))
	}
	vendorID, ok := parsed.find("Vendor ID:")
	if !ok {
		return reportErr(errors.New("vendor ID not found"))
	}
	...
	default:
		return reportErr(errors.Errorf("unknown vendor ID: %q", vendorID))
	}
}
...
func findArmSOC(parsed *lscpuResult) (device.Config_SOCVendor, device.Config_SOC, error) {
	...
	if !ok {
		return reportErr(errors.New("ARM model not found"))
	}
	...
	case "4":
		...
		if !ok {
			return reportErr(errors.New("unknown number of cores"))
		}
		switch cpus {
		case "8":
		...
		default:
			return reportErr(errors.Errorf("unknown ARM SOC (model=%s, cpus=%s)", model, cpus))
		}
	default:
		return reportErr(errors.Errorf("unknown ARM model: %s", model))
	}
}
...
func findIntelSOC(parsed *lscpuResult) (device.Config_SOCVendor, device.Config_SOC, error) {
	...
	if family, ok := parsed.find("CPU family:"); !ok {
		return reportErr(errors.New("Intel family not found"))
	} else if family != "6" {
		return reportErr(errors.Errorf("unknown Intel family: %s", family))
	}

	modelStr, ok := parsed.find("Model:")
	if !ok {
		return reportErr(errors.New("Intel model not found"))
	}
	model, err := strconv.ParseInt(modelStr, 10, 64)
	if err != nil {
		return reportErr(errors.Wrapf(err, "failed to parse intel model: %q", modelStr))
	}
	...
	case INTEL_FAM6_KABYLAKE_L:
		...
		if !ok {
			return reportErr(errors.New("Intel model name not found"))
		}
		...
		return reportErr(errors.Errorf("unknown model name: %s", modelName))
	...
	default:
		return reportErr(errors.Errorf("unknown Intel model: %d", model))
	}
}
...
func findAMDSOC(parsed *lscpuResult) (device.Config_SOCVendor, device.Config_SOC, error) {
	...
	model, ok := parsed.find("Model:")
	if !ok {
		return reportErr(errors.New("AMD model not found"))
	}
	if model == "112" {
		return vendor, device.Config_SOC_STONEY_RIDGE, nil
	}
	return reportErr(errors.Errorf("unknown AMD model: %s", model))
}
