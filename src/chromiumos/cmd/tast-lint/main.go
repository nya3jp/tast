...
// isNewlyAddedPackage returns true if given directory is newly added package.
// A directory of newly added package satisfies the following conditions.
...
func isNewlyAddedPackage(dir string, ptdfmap *map[string][]git.CommitFile) (bool, error) {
	...
	sameLen := (numExGofiles == len((*ptdfmap)[dir]))
	allAdded := true
	for _, path := range (*ptdfmap)[dir] {
		if path.Status != git.Added {
			allAdded = false
			break
		}
	}
	return sameLen && allAdded, nil
}
...
func checkAll(g *git.Git, paths []git.CommitFile, debug bool) ([]*check.Issue, error) {
	...
	for dir, cfs := range dfmap {
		...
		isNewlyAdded, err := isNewlyAddedPackage(dir, &dfmap)
		if err != nil {
			return nil, err
		}
		if isNewlyAdded {
