...
type localTestData struct {
	...
	systemLog string // simulated content of system log for the initial boot (defaultBootID)
...
func (td *localTestData) handleExec(req *sshtest.ExecReq) {
	...
	case "exec sync":
		req.Start(true)
		req.End(0)
	case "exec croslog --source=journal --quiet --boot=" + strings.Replace(defaultBootID, "-", "", -1) + " --line=1000":
		req.Start(true)
		io.WriteString(req, td.systemLog)
		req.End(0)
