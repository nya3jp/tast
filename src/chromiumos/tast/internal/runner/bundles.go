...
func getBundlesAndTests(args *Args) (bundles []string, tests []*testing.EntityListTestInfo, err *command.StatusError) {
	var glob string
	switch args.Mode {
	case RunTestsMode:
...
func getTests(args *Args, bundles []string) (tests []*testing.EntityListTestInfo,
	...
	for _, b := range bundles {
		...
		go func() {
			...
			ts := make([]*testing.EntityListTestInfo, 0)
	...
	bundleTests := make(map[string][]*testing.EntityListTestInfo)
