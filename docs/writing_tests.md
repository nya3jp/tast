...
service Boot {
  ...
  // successful boot.
  rpc Boot (BootRequest) returns (google.protobuf.Empty) {}
}
...
*   _File names_: Name .proto files in the same way as [test `.go` files].
    For example, a service named `TPMStress` should be defined in
    `tpm_stress.proto`.
*   _Package names_: Protocol buffer package name specified in the `package`
...
*   _Service names_: Name services in the same way as [test functions]. Service
    names should not end with `Server`, `Service` or `Client`; Go's generated
    binding provides utility functions named `NewFooBarServer` and
    `NewFooBarClient` for `FooBar` service, so naming services with such
    suffixes will generate odd named utilities.
*   _Message names_: Method request/response messages should be named
    `FooBarRequest`/`FooBarResponse`.
*   _Comments_: Write comments in the [godoc style] since these protocol buffers
    are used only by Tast tests in Go.
...
func (*Boot) Boot(ctx context.Context, req *pb.BootRequest) (*empty.Empty, error) {
    ...
}
```
...
*   _File names_: Name gRPC service implementation `.go` files in the similar
    way as [test `.go` files], but with `_service.go` suffix. For example,
    a service named TPMStress is implemented in `tpm_stress_service.go`.
*   _Implementation type_: A type implementing gRPC service should have exactly
    the same name as the service name. The implementing type should be the only
    exported symbol in the `_service.go` file. Exactly one gRPC service
    implementation should be registered in a single file.
*   _Inter-file references_: Similarly to test files, `_service.go` file should
    not refer symbols in different files in the same directory. Consequently,
    a gRPC service has to be implemented in a single file. If the file gets
    too long, please consider introducing a subpackage just like tests.
...
req := pb.BootRequest{Impl: pb.BootRequest_VM}
var res pb.BootResponse
