...
func TestLog(t *gotesting.T) {
	var out outputSink
	root := NewTestRoot(&TestInstance{Timeout: time.Minute}, &out, &TestConfig{})
	s := root.newTestState()
	s.Log("msg ", 1)
	s.Logf("msg %d", 2)
...
func TestDataPathDeclared(t *gotesting.T) {
	...
	for _, tc := range []struct{ in, exp string }{
		...
		var out outputSink
		root := NewTestRoot(&test, &out, &TestConfig{DataDir: dataDir})
		s := root.newTestState()
...
func TestDataPathNotDeclared(t *gotesting.T) {
	...
	root := NewTestRoot(&test, &out, &TestConfig{DataDir: "/data"})
	s := root.newTestState()
...
func TestDataFileServer(t *gotesting.T) {
	...
	var out outputSink
	root := NewTestRoot(&test, &out, &TestConfig{DataDir: td})
	s := root.newTestState()
...
func TestCloudStorage(t *gotesting.T) {
	...
	var out outputSink
	root := NewTestRoot(&TestInstance{Name: "example.Test"}, &out, &TestConfig{CloudStorage: want})
	s := root.newTestState()
	got := s.CloudStorage()
