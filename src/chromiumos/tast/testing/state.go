...
func newState(ctx context.Context, test *Test, ch chan Output, cfg *TestConfig) *State {
	// If the Meta struct should be made available to the test, clone it to make sure the test doesn't modify the original.
	if cfg.Meta != nil {
		if parts := strings.SplitN(test.Name, ".", 2); len(parts) == 2 && parts[0] == metaCategory {
			cfg.Meta = cfg.Meta.clone()
		} else {
			cfg.Meta = nil
		}
	}
...
func (s *State) Meta() *Meta { return s.cfg.Meta }
