...
```
func GetMinimizedWindow(ctx context.Context, tconn *chrome.TestConn) (*Window, error) {
	return FindWindow(ctx, tconn, func(window *Window) bool {
		return window.State == ash.WindowStateType.WindowStateMinimized
	})
}

func someFunction() error {
	...
	w, err := GetMinimizedWindow(ctx, tconn);
	if err != nil {
		if errors.Is(err, ash.ErrorWindowNotFound) {
			return nil
		}
		return err
	}
...
var found bool
if err := conn.Eval(ctx, code, &foundIt); err != nil {
	// Do error handling here.
}
```
...
In the test we start a HTTP server as a ```http.FileServer``` which serves requests for the files located in the folder given as argument. The used folder, ```s.DataFileSystem()```, is the folder where additional files for the test are copied to on the DuT, which is where our files for the website end up. Then we open the website in a new Chrome window.
