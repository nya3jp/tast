...
// startBundleCmd creates and returns a new command running the test bundle at path.
// cmd's Start method has already been called, and the caller is responsible for calling wait.
// The returned stdin value is a pipe that will be connected to the process's stdin.
// A new session is created for the bundle process.
func startBundleCmd(path string, stdout, stderr io.Writer) (cmd *exec.Cmd, stdin io.WriteCloser, err error) {
...
func runBundle(path string, args *bundle.Args, stdout io.Writer) *command.StatusError {
	...
	stderr := bytes.Buffer{}
	cmd, stdin, err := startBundleCmd(path, stdout, &stderr)
	if err != nil {
