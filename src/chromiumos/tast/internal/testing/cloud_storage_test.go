...
func TestCloudStorageTLWOpenMissing(t *testing.T) {
	...
	stopFunc, addr := faketlw.StartWiringServer(t, faketlw.WithCacheFileMap(
		map[string][]byte{}, // no file served
	), faketlw.WithDUTName(dutName))
	defer stopFunc()
	...
	r, err := cs.Open(context.Background(), "gs://a/b/a")
	if err == nil {
		r.Close()
		t.Fatal("Open succeeded unexpectedly")
	}
	// The underlying error comes from CacheForDUT TLW RPC.
	// TODO(crbug.com/1112616): Handle is-not-exist error if possible.
	// if !os.IsNotExist(err) {
	// 	t.Fatal("Open failed: ", err)
	// }
}
