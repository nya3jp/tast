...
//
// emerge prints each missing dependency to stdout on a line similar to the following (sometimes
// with trailing whitespace or additional information after the package name):
//
...
func parseEmergeOutput(stdout, stderr []byte, pkg string) (missingDeps []string, masked bool) {
	depRegexp := regexp.MustCompile(`^\[ebuild[^]]*\]\s+(\S+/\S+)(?:\s|$)`)
