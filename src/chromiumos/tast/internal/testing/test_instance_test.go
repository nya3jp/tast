...
func TestInstantiate(t *gotesting.T) {
	...
	want := []*TestInstance{{
		...
		Data:         []string{"data1.txt", "data2.txt"},
		AllVars:      []string{"var1", "vardep1"}, // union of Test.Vars and Test.VarDeps
		VarDeps:      []string{"vardep1"},
		SoftwareDeps: []string{"dep1", "dep2"},
		Timeout:      123 * time.Second,
		ServiceDeps:  []string{"svc1", "svc2"},
...
func TestVarDeps(t *gotesting.T) {
	...
	for _, tc := range []struct {
		...
		want: &ShouldRunResult{
			Errors: []string{"runtime variable foo is missing and doesn't match with <nil>"},
		},
	}, {
		name: "skip",
		features: &dep.Features{
		...
		want: &ShouldRunResult{
			SkipReasons: []string{"runtime variable foo is missing and matches with foo"},
		},
	}} {
