...
func (c *Config) SetFlags(f *flag.FlagSet) {
	...
	f.StringVar(&c.buildWorkspace, "buildworkspace", "", "path to Go workspace containing test bundle source code")
	...
	if c.mode == RunTestsMode {
		...
		f.StringVar(&c.overlayDir, "overlaydir", "", "base overlay directory containing test bundle ebuild")
