...
The chrome.Automation library uses the Chrome Accesibilty Tree to view and
...
That begin said, it does not have access to UI elements in containers or VMs
...
chrome.LoggedIn() precodition and then force the test to fail and give you a
dump of the Accesibility tree. That way you can look at the tree and decide what
...
# Interacting with the Accessability Tree
...
can use to find it, so lets find and right click that class:
```go
...
asynchronus and might not immeadiately update the UI tree.
...
Now that the wallpaper picker is open, lets set the background to a solid color.
...
we have to decide what we can and will considered a success. A good solution
