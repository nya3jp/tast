...
type State struct {
	...
	hasError bool // true if the current subtest has encountered an error
	// the test fails if this is true for the inital subtest
...
func newState(test *TestCase, ch chan<- Output, cfg *TestConfig) *State {
	return &State{root: &rootState{test: test, ch: ch, cfg: cfg}}
}
...
// is stopped; its parent continues. Run must not be called in parallel.
...
func (s *State) Run(ctx context.Context, name string, run func(context.Context, *State)) bool {
	...
	go func() {
		...
		s.Logf("Starting subtest %s", name)
		run(ctx, ns)
	}()
