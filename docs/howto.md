...
```
The [chrome.Eval()] can also be used on the connection to tast's test extension which gives access to other APIs. A connection can be created with [chrome.TestAPIConn()]. The connection to tast's test extension should not be closed as it is shared.
