...
func TestHardwareDepsCel(t *gotesting.T) {
	for _, c := range []struct {
		...
		{hwdep.D(hwdep.TouchScreen()), "scope.hardware_topology.screen.touch_support == scope.hardware_features.PRESENT"},
		{hwdep.D(hwdep.Fingerprint()), "scope.hardware_topology.fingerprint.location != scope.hardware_topology.fingerprint.NOT_PRESENT"},
		{hwdep.D(hwdep.InternalDisplay()), "scope.hardware_topology.screen.milliinch.value != 0"},
		...
		{hwdep.D(hwdep.TouchScreen(), hwdep.Fingerprint()),
			"scope.hardware_topology.screen.touch_support == scope.hardware_features.PRESENT && scope.hardware_topology.fingerprint.location != scope.hardware_topology.fingerprint.NOT_PRESENT"},
	} {
		actual := c.input.Cel()
		if actual != c.expected {
			t.Errorf("TestHardwareDeps(%v): got %q; want %q", c.input, actual, c.expected)
		}
	}
}
