...
func getTests(args *Args, bundles []string) (tests []*testing.TestInstance,
	...
	for _, b := range bundles {
		...
		go func() {
			out := bytes.Buffer{}
			fd := int(os.Stdout.Fd())
			if err := runBundle(bundle, bundleArgs, &out, fd); err != nil {
...
func runBundle(path string, bundleArgs *bundle.Args, stdout io.Writer, stdoutFD int) *command.StatusError {
	...
	stdoutWatcher, err := newPipeWatcher(stdoutFD)
	if err != nil {
...
func handleDownloadPrivateBundles(ctx context.Context, args *Args, cfg *Config, stdout io.Writer) error {
	...
	lf(fmt.Sprintf("Downloading private bundles from %s", archiveURL))
