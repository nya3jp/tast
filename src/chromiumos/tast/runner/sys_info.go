...
const (
	...
	journaldCompactLog = "journald.log"        // compact human-readable journald log
	journaldExportLog  = "journald_export.log" // full journald log with metadata
)
...
func handleGetSysInfoState(ctx context.Context, args *Args, w io.Writer) error {
...
func handleCollectSysInfo(ctx context.Context, args *Args, w io.Writer) error {
	if args.SystemLogDir == "" || len(args.SystemCrashDirs) == 0 {
		return command.NewStatusErrorf(statusBadArgs, "system info collection unsupported")
	}
	...
	if subdir := args.JournaldSubdir; subdir != "" {
