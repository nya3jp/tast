...
func (c *DevserverClient) Download(ctx context.Context, w io.Writer, gsURL string) error {
	...
	if len(c.selected) == 0 {
		return errors.New("no devserver is up")
	}
...
func (c *DevserverClient) downloadFrom(ctx context.Context, w io.Writer, addr, bucket, path string) error {
	staticURL := url.URL{
		Scheme: "http",
		Host:   addr,
		Path:   "/static/" + path,
