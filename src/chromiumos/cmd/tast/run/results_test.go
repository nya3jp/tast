...
func TestReadTestOutput(t *gotesting.T) {
	...
	results, unstartedTests, _, err := readTestOutput(context.Background(), &cfg, &b, os.Rename, nil)
	if err != nil {
...
func TestReadTestOutputTimingLog(t *gotesting.T) {
	...
	if _, _, _, err := readTestOutput(ctx, &cfg, &b, os.Rename, nil); err != nil {
		t.Fatal("readTestOutput failed: ", err)
	}
...
func TestWritePartialResults(t *gotesting.T) {
	...
	results, unstarted, _, err := readTestOutput(context.Background(), &cfg, &b, os.Rename, nil)
	if err == nil {
