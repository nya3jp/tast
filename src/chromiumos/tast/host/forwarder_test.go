...
func TestForwarder(t *testing.T) {
	...
	// forwarded connectinos, so we'll swap in a function later that just returns
	// one end of a pipe to simulate creating remote connections.
	fwd, err := NewForwarder(nil, "127.0.0.1:0", "192.0.2.1:12345")
	if err != nil {
...
func TestForwarderError(t *testing.T) {
	...
	// and configure an error handler that copies errors to a channel.
	connErr := errors.New("intentional error")
	fwd.connFunc = func() (net.Conn, error) { return nil, connErr }
	ch := make(chan error, 1)
	fwd.SetErrorFunc(func(err error) { ch <- err })
