...
Running Tast on a device for the first time will remove all user profiles from the
...
**WARNING: Potential data loss:** Running Tast on a device for the first time will
remove all existing user profiles, including any local state.
