...
func handleGetDUTInfo(args *Args, cfg *Config, w io.Writer) error {
	...
	if args.GetDUTInfo.RequestDeviceConfig {
		var ws []string
		dc, hwFeatures, ws = newDeviceConfig()
...
func newDeviceConfig() (dc *device.Config, features *configpb.HardwareFeatures, warns []string) {
	...
	if hasTouchScreen {
		dut.Screen.TouchSupport = configpb.HardwareFeatures_PRESENT
	}
