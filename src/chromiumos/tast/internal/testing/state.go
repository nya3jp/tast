...
// Entities include tests, fixtures and services. An entity must be registered
// to the framework by calling testing.Add* at the test bundle initialization
// time. Entity metadata contain various information the framework needs to know
// to run an entity properly. For example, an entity name is used to mention it
// in other entities' metadata and command line arguments, dependencies
// (data files, softwares, hardwares, ...) specify requirements that must be
// satisfied before running an entity, and so on. When a test bundle is started,
// the framework builds an execution plan of entities according to the request,
// prepare necessary dependencies, and call into entity entry points.
//
// Entry points are functions that take two arguments: context.Context and
// testing.*State (the exact State type varies by the entry point type).
// context.Context passed to an entry point is associated with an entity, thus
// it can be passed to testing.Context* to query the corresponding entity
// metadata (e.g. testing.ContextServiceDeps) or emit logs for the entity
// (testing.ContextLog). A State type provides API methods that interact with
// the framework. Though the type was named "State" for historical reasons, it
// is not something that keeps track of the state of the current entity
// execution as you might imagine. If we were able to rename them, we might want
// to name them "TestAPI" or similar, but now that we've got a lot of tests it
// seems not worth doing.
//
// There are a few types of entry points that are registered to the framework
// with no metadata, called semi-entities (e.g. preconditions and hooks).
// Semi-entity entry points are called with context.Context and testing.*State
// associated with different entities at run time (e.g. context.Context passed
// to a precondition entry point is associated with a test that depends on the
// precondition).
//
...
// object (and additionally a TestEntityRoot object in the case of a test) is
// allocated. Root objects are private to the framework, and user code always
