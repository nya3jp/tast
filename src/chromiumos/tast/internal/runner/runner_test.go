...
func runFakeBundle() int {
	...
	return bundle.Local(nil, os.Stdin, os.Stdout, os.Stderr, bundle.LocalDelegate{})
}
...
func TestFilterSkippedTests(t *gotesting.T) {
	args := &Args{
		Mode: RunTestsMode,
		RunTests: &RunTestsArgs{
			BundleArgs: bundle.RunTestsArgs{
				CheckSoftwareDeps:         false,
				AvailableSoftwareFeatures: []string{"linux", "kernel3x"},
			},
		},
	}
	...
	got := names(filterSkippedTests(args, tests))
	want := []string{"aaa", "bbb"}
	if !reflect.DeepEqual(got, want) {
		t.Errorf("filterSkippedTests(%q, CheckSoftwareDeps=false) = %q; want %q", names(tests), got, want)
	}
	...
	want = []string{"bbb"}
	if !reflect.DeepEqual(got, want) {
		t.Errorf("filterSkippedTests(%q, CheckSoftwareDeps=true) = %q; want %q", names(tests), got, want)
	}
}
