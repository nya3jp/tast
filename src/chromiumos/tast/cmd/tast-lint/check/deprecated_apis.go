...
func deprecatedAPIs(fs *token.FileSet, f *ast.File, ds []*deprecatedAPI) []*Issue {
	...
	importSpec := make(map[string]*ast.ImportSpec) // key: package identifier
	for _, i := range f.Imports {
		sel := filepath.Base(unquote(i.Path.Value))
		if i.Name != nil {
			sel = i.Name.Name
		}
		importSpec[sel] = i
	...
	astutil.Apply(f, func(c *astutil.Cursor) bool {
		...
		i, ok := importSpec[x.Name]
		if !ok {
			return true
		}
		m, ok := deprecatedPkgIdent[unquote(i.Path.Value)]
		if !ok {
			return true
		}
		d, ok := m[sel.Sel.Name]
