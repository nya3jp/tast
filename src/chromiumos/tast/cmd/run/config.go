...
type Config struct {
	...
	ResDir string
	// PrintMode controls whether tests should be printed to stdout instead of being executed.
	PrintMode PrintMode
...
func (c *Config) SetFlags(f *flag.FlagSet, trunkDir string) {
	...
	// Only create a results dir if we're running tests rather than printing them.
