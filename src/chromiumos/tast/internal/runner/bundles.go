...
func getBundlesAndTests(args *Args) (bundleAndTests []testsPerBundle,
	tests []*testing.EntityInfo, startIndex, endIndex int, err *command.StatusError) {
	var glob string
	switch args.Mode {
	case RunTestsMode:
	...
	default:
		return nil, nil, 0, 0, command.NewStatusErrorf(statusBadArgs, "bundles unneeded for mode %v", args.Mode)
	}
	...
	tests, startIndex, endIndex, bundleAndTests, err = getTests(args, bundles)
	return bundleAndTests, tests, startIndex, endIndex, err
}
...
func getTests(args *Args, bundles []string) (tests []*testing.EntityInfo,
	...
	if err != nil {
		return nil, 0, 0, nil, command.NewStatusErrorf(statusBadArgs, "%v", err)
	}
