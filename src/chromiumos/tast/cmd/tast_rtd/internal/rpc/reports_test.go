...
func TestReportsServer_LogStream(t *testing.T) {
	const (
		logSinkNameTmpl      = "/tmp/tast/results/latest/tests/name01-20210123/%s/log.txt"
		testName1            = "test.Name1"
		requestName1         = "request_for_test_name1"
		testLog1a            = "log1a"
		testLog1b            = "log1b"
		expectedLogSinkName1 = "/tmp/tast/results/latest/tests/name01-20210123/test.Name1/log.txt"
		testName2            = "test.Name2"
		requestName2         = "request_for_test_name2"
		testLog2a            = "log2a"
		expectedLogSinkName2 = "/tmp/tast/results/latest/tests/name01-202010123/test.Name2/log.txt"
	)
	...
	srv, err := NewReportsServer(0, psClient, testsToRequests, logSinkNameTmpl)
	if err != nil {
	...
	if s := string(psServer.ReceivedLog(expectedLogSinkName1, requestName1)); !strings.Contains(s, testLog1a+testLog1b) {
		t.Errorf("Log sent to test #1 was not forwarded. Log#1=%q", s)
	}
	if s := string(psServer.ReceivedLog(expectedLogSinkName1, requestName1)); strings.Contains(s, testLog2a) {
		t.Errorf("Log sent to #2 appeared in #1. Log#1=%q", s)
	}
	if s := string(psServer.ReceivedLog(expectedLogSinkName2, requestName2)); strings.Contains(s, testLog1a) {
...
func TestReportsServer_ReportResult(t *testing.T) {
	...
	reportsServer, err := NewReportsServer(0, psClient, testsToRequests, "/tmp/tast/results/tests/%s/log.txt")
	if err != nil {
		t.Fatalf("Failed to start Reports server: %v", err)
	}
