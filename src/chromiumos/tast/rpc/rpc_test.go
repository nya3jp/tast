...
func TestRPCSuccess(t *gotesting.T) {
	ctx := entity.WithCurrentEntity(context.Background(), &entity.CurrentEntity{})
	called := false
	...
	callCtx := entity.WithCurrentEntity(ctx, &entity.CurrentEntity{
...
func TestRPCForwardCurrentEntity(t *gotesting.T) {
	...
	pp := newPingPair(ctx, t, func(ctx context.Context, s *testing.ServiceState) error {
		called = true
		deps, depsOK = entity.ContextSoftwareDeps(ctx)
		return nil
	})
	defer pp.Close(ctx)
...
func TestRPCForwardLogs(t *gotesting.T) {
	...
	ctx = entity.WithCurrentEntity(ctx, &entity.CurrentEntity{})
