...
When running multiple tests which requires `chrome_login`, login and logout will
happen between all tests, which are time-consuming. You can skip them by
specifying `Pre` field in `testing.Test`.
For example, if you set `Pre` field like this, this test can share the same
Chrome instance with other tests having the same precondition.
...
Existing preconditions are listed in the [Chrome precondition] document.
...
[Chrome precondition]: precondition.md
