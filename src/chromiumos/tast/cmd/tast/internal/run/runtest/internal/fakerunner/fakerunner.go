...
func (s *testService) RunTests(client protocol.TestService_RunTestsServer) error {
	initReq, err := client.Recv()
	if err != nil {
		return err
	}
	s.cfg.OnRunTestsInit(initReq.GetRunTestsInit())

	server, err := s.cl.RunTests(client.Context())
	if err != nil {
		return err
	}
	defer server.CloseSend()

	if err := server.Send(initReq); err != nil {
		return err
	}
