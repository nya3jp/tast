...
func TestGetMinidumpReleaseInfo(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			verifyFunc: func(t *testing.T, ri *MinidumpReleaseInfo) {
				if board := ri.CrashpadAnnotations["lsb-release-board"]; board != "betty" {
					t.Errorf("Expected lsb-release-board = betty, got: %s", board)
				}
				if builderPath := ri.CrashpadAnnotations["lsb-release-builder-path"]; builderPath != "betty-release/R93-14040.0.0" {
