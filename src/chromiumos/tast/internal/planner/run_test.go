...
func TestRunSkipStages(t *gotesting.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			want: []control.Msg{
				&control.EntityStart{Info: testing.EntityInfo{Name: "0", Timeout: time.Minute, Bundle: filepath.Base(os.Args[0])}},
		...
		},
		{
			...
			want: []control.Msg{
				...
				&control.EntityLog{Name: "0", Text: "postTest: OK"},
				&control.EntityEnd{Name: "0"},
				&control.EntityStart{Info: testing.EntityInfo{Name: "1", Timeout: time.Minute, Bundle: filepath.Base(os.Args[0])}},
