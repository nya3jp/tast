...
func TestRunConfig(t *gotesting.T) {
	...
	if wrapper.runCfg.RawTarget != target {
		t.Errorf("runCmd.Execute(%v) passed target %q; want %q", args, wrapper.runCfg.RawTarget, target)
	}
...
func TestRunWriteResultsWithZeroTests(t *gotesting.T) {
	...
	wrapper := stubRunWrapper{
		runRes:    []*jsonprotocol.EntityResult{}, // no tests were executed
