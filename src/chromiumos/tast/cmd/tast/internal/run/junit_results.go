...
type TestSuite struct {
	...
	Time      string `xml:"time,attr"`      // duration, in seconds (with a decimal point)
...
func ToJUnitResults(results []*EntityResult) ([]byte, error) {
	...
	for _, r := range results {
		testCase := TestCase{
			...
			Timestamp: r.Start.Format("2006-01-02Z15:04:05"),
	...
	suite.Timestamp = suite.TestCase[0].Timestamp
	startTime := results[0].Start
	endTime := results[len(results)-1].End
	suite.Time =
		fmt.Sprintf("%.1f", endTime.Sub(startTime).Seconds())
	if len(results) != 0 {
