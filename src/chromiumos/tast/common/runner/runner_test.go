...
func TestCopyTestOutput(t *gotesting.T) {
	...
	if m, err := r.ReadMessage(); err != nil {
		t.Error("Failed to read first message: ", err)
	} else if lm, ok := m.(*control.TestLog); !ok {
		t.Error("Failed to decode first message")
	} else {
		exp := &control.TestLog{Time: t1, Text: msg}
		if !reflect.DeepEqual(lm, exp) {
			t.Errorf("First message is %v; want %v", lm, exp)
		}
	}

	if m, err := r.ReadMessage(); err != nil {
		t.Error("Failed to read second message: ", err)
	} else if em, ok := m.(*control.TestError); !ok {
		t.Error("Failed to decode second message")
	} else {
		exp := &control.TestError{Time: t2, Error: e}
		if !reflect.DeepEqual(em, exp) {
			t.Errorf("Second message is %v; want %v", em, exp)
		}
	}
