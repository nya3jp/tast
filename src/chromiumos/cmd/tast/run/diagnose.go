...
func diagnoseReboot(ctx context.Context, cfg *Config) string {
	...
	return diagnoseByLogs(journal, ramOops)
}
...
// diagnoseReboot diagnoses the target reboot during local test runs by logs.
// journal is the systemd journal before the reboot, and ramOops is the kernel log
// obtained from console-ramoops.
func diagnoseByLogs(journal, ramOops string) string {
