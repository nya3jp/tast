...
For `Pre`, each parametrized test can define its own `Pre` as long as it is not
defined in the enclosing test. If `Pre` is only defined in the enclosing test,
it will be inherited by the parametrized tests.

If more than one parametrized test define `Pre`, these `Pre` must return the
same value type. E.g: It is Ok if a parametrized test uses `arc.Booted()` and
another one uses the hypothetical `arcvm.Booted()` precondition, since both
