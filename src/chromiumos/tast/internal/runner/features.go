...
func newDeviceConfigAndHardwareFeatures() (retDeviceInfo *protocol.DeprecatedDeviceConfig, retFeatures *configpb.HardwareFeatures, warns []string) {
	...
	deviceInfo := &protocol.DeprecatedDeviceConfig{
	...
	func() {
		...
		deviceInfo.Power = protocol.DeprecatedDeviceConfig_POWER_SUPPLY_UNSPECIFIED
		...
		for _, file := range files {
			...
			if strings.HasPrefix(supplyType, "Battery") {
				...
				deviceInfo.Power = protocol.DeprecatedDeviceConfig_POWER_SUPPLY_BATTERY
			...
			deviceInfo.Power = protocol.DeprecatedDeviceConfig_POWER_SUPPLY_AC_ONLY
		}
	}()
	...
	return deviceInfo, features, warns
}
