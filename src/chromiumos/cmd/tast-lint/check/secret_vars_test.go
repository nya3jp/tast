...
const invalidVarNameErrTmpl = "%s: Var name %s violates naming convention; it should match %s[a-zA-Z][0-9A-Za-z_]*"
...
func TestInvalidFileNames(t *testing.T) {
	...
	for _, path := range paths {
		expects := []string{
			path + `: File's basename doesn't match expected regex ^([a-zA-Z]+(?:\.[a-zA-Z]+)?\.)yaml$`,
		}
		issues := SecretVarFile(path, []byte(data))
		verifyIssues(t, issues, expects)
	}
