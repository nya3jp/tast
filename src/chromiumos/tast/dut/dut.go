...
type DUT struct {
	sopt       ssh.Options
	hst        *ssh.Conn
	rebootHook func(context.Context, *DUT) error
}
...
func New(target, keyFile, keyDir string, rebootHook func(context.Context, *DUT) error) (*DUT, error) {
	d := DUT{rebootHook: rebootHook}
...
func (d *DUT) Reboot(ctx context.Context) error {
	if d.rebootHook != nil {
		if err := d.rebootHook(ctx, d); err != nil {
