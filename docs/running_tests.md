...
*   `system_logs/` - Diff of `/var/log` on the DUT before and after testing.
    *   `combined/` - Combined log collected from system logs.
        *   `combined.log` - Human-readable system log messages.
        *   `combined.export.gz` - gzip-compressed logs with full metadata from
