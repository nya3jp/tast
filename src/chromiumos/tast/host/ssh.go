...
func (s *SSH) PutTreeRename(ctx context.Context, srcDir, dstDir string,
	...
	if err := s.Command("mkdir", "-p", dstDir).Run(ctx); err != nil {
		return 0, fmt.Errorf("failed to mkdir %s: %v", dstDir, err)
	}

	rcmd := s.Command("tar", "-x", "--gzip", "--no-same-owner", "--recursive-unlink", "-C", dstDir)
	pw, err := rcmd.StdinPipe()
	if err != nil {
		return 0, fmt.Errorf("failed to open stdin pipe: %v", err)
	}
	if err := rcmd.Start(ctx); err != nil {
		return 0, fmt.Errorf("running remote tar failed: %v", err)
	}
	defer rcmd.Wait(ctx)
	defer rcmd.Abort()
	...
	if bytes, err = io.Copy(pw, pr); err != nil {
		return 0, fmt.Errorf("copying from local to remote tar failed: %v", err)
	}
	if err := pw.Close(); err != nil {
		return 0, fmt.Errorf("failed to close pipe: %v", err)
	}
	if err := rcmd.Wait(ctx); err != nil {
