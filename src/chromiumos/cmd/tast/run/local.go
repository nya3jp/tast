...
const (
	...
	localBundleBuiltinDir    = "/usr/local/share/tast/bundles"        // on-device dir with preinstalled test bundles
	localBundlePushDir       = "/usr/local/share/tast/bundles_pushed" // on-device dir with test bundles pushed by tast command
	...
	if cfg.PrintMode == DontPrint {
		...
		if paths, err = getDataFilePaths(ctx, cfg, hst, bundleGlob); err != nil {
			...
			if err = buildAndPushLocalRunner(ctx, cfg, hst); err != nil {
				return "", fmt.Errorf("failed to deploy test runner: %v", err)
			}
			if paths, err = getDataFilePaths(ctx, cfg, hst, bundleGlob); err != nil {
				return "", fmt.Errorf("failed to get data file list: %v", err)
			}
...
func localRunnerExists(ctx context.Context, hst *host.SSH) (bool, error) {
	cmd := fmt.Sprintf("if [ -e %s ]; then echo -n exists; fi", host.QuoteShellArg(localRunnerPath))
	out, err := hst.Run(ctx, cmd)
	if err != nil {
		return false, err
	}
	return string(out) == "exists", nil
}
...
func buildAndPushLocalRunner(ctx context.Context, cfg *Config, hst *host.SSH) error {
	...
	if out, err := build.Build(ctx, &bc, localRunnerPkg, src, "build_runner"); err != nil {
		return fmt.Errorf("failed building test runner: %v\n\n%s", err, out)
	}
