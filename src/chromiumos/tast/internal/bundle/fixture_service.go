...
func (*fixtureService) pushAndPop(srv FixtureService_RunFixtureServer) (retErr error) {
	sendDone := func() error {
	...
	// sendDone for the pop operation runs after all other deferred operations
	// are done. This resolves timing issues in the unit test
	// TestFixtureServiceDefaultTempDir.
	defer func() {
		if retErr != nil {
			return
		}
		if err := sendDone(); err != nil && retErr == nil {
			retErr = err
		}
	}()
	...
	if err := func() error { // push temporarily setting temp dir
