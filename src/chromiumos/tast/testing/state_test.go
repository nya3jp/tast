...
func TestLog(t *gotesting.T) {
	...
	s := NewState(context.Background(), &Test{Timeout: time.Second}, ch, "", "")
	s.Log("msg ", 1)
	s.Logf("msg %d", 2)
	close(ch)
	out := readOutput(ch)
...
func TestDataPathValid(t *gotesting.T) {
	...
	for _, tc := range []struct{ in, exp string }{
		...
		s := NewState(context.Background(), &Test{Timeout: time.Second}, make(chan Output), dataDir, "")
...
func TestDataPathNoParent(t *gotesting.T) {
	...
	s := NewState(context.Background(), &Test{Timeout: time.Second}, ch, "/data", "")
