...
func testsToRun(cfg *runConfig, patterns []string) ([]*testing.RunnableTest, error) {
...
func (ew *eventWriter) TestStart(t *testing.RunnableTest) error {
	ew.testName = t.Name
	if ew.lg != nil {
...
func (ew *eventWriter) TestEnd(t *testing.RunnableTest, missingDeps []string, timingLog *timing.Log) error {
	ew.testName = ""
	if ew.lg != nil {
...
func runTests(ctx context.Context, stdout io.Writer, args *Args, cfg *runConfig,
	bt bundleType, tests []*testing.RunnableTest) error {
	...
	var rt *testing.RunnableTest                           // last-seen test that will be run
	missingDeps := make([][]string, len(tests))            // per-test missing deps, in same order as tests
	nextTests := make([]*testing.RunnableTest, len(tests)) // test to run after each test, in same order as tests
...
func runTest(ctx context.Context, ew *eventWriter, args *Args, cfg *runConfig,
	t, next *testing.RunnableTest, rd *testing.RemoteData) {
	ew.TestStart(t)
...
func reportSkippedTest(ctx context.Context, ew *eventWriter, args *Args,
	t *testing.RunnableTest, missingDeps []string) {
	ew.TestStart(t)
