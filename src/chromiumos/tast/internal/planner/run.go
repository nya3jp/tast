...
func runFixtTree(ctx context.Context, tree *fixtTree, stack *fixtureStack, pcfg *Config, out OutputStream, dl *downloader) error {
	for !tree.Empty() {
		if err := func() error {
			...
			// Push a fixture to the stack, calling its SetUp if the fixture stack is green.
			...
			for stack.Status() != statusOrange && len(tree.tests) > 0 {
				t := tree.tests[0]
			...
			for stack.Status() != statusOrange && len(tree.children) > 0 {
			...
			// Pop the fixture from the stack, calling its TearDown if it is not red.
...
func runTest(ctx context.Context, t *testing.TestInstance, tout *entityOutputStream, pcfg *Config, precfg *preConfig, stack *fixtureStack, dl *downloader) error {
	...
	case statusGreen:
	case statusRed:
		msg := fmt.Sprintf("[Fixture failure] %s: SetUp failed", stack.OrangeFixtureName())
		tout.Error(testing.NewError(nil, msg, msg, 0))
		return nil
	case statusOrange:
		return errors.New("BUG: Cannot run a test on an orange fixture stack")
	}
...
func runTestWithRoot(ctx context.Context, t *testing.TestInstance, root *testing.TestEntityRoot, pcfg *Config, precfg *preConfig) error {
	...
	if err := safeCall(ctx, t.Name, preTestTimeout, defaultExitTimeout, errorOnPanic(testState), func(ctx context.Context) {
	...
	if !root.HasError() && t.Pre != nil {
		...
		if err := safeCall(ctx, t.Pre.String(), t.Pre.Timeout(), defaultExitTimeout, errorOnPanic(preState), func(ctx context.Context) {
	...
	if !root.HasError() {
		if err := safeCall(ctx, t.Name, t.Timeout, timeoutOrDefault(t.ExitTimeout, defaultExitTimeout), errorOnPanic(testState), func(ctx context.Context) {
	...
	if postTestFunc != nil {
		if err := safeCall(ctx, t.Name, postTestTimeout, defaultExitTimeout, errorOnPanic(testState), func(ctx context.Context) {
