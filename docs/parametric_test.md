...
Registration of a parametric test looks like the following:
...
All `Name`s in a `Params` array should be unique.
`Name` can be empty (or can be ommitted). In this case, no suffix (including a leading `.`)
will be appended. Because of the uniqueness requirement, a `Params` array
...
`interface{}`, the returned value should be type-asserted to some
...
`Params` should be an array literal containing `Param` struct literals.
