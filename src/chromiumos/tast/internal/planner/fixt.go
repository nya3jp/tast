...
// Status colors are borrowed from those of traffic light.
...
func (s fixtureStatus) String() string {
	switch s {
	case statusRed: return "red"
	case statusGreen: return "green"
	case statusYellow: return "yellow"
...
//
//  1. When there is an yellow fixture in the stack, no other fixtures are red.
...
// YellowFixtureName returns a name of an yellow fixture in the fixture stack.
//
// If there is no yellow fixture in the stack, an empty string is returned.
func (st *fixtureStack) YellowFixtureName() string {
	for _, f := range st.stack {
		if f.Status() == statusYellow {
			return f.Name()
		}
	}
	return ""
...
//
// It is an error to call Push for an yellow fixture stack.
func (st *fixtureStack) Push(ctx context.Context, fixt *testing.Fixture) error {
	status := st.Status()
	if status == statusYellow {
		return errors.New("BUG: fixture must not be pushed to an yellow stack")
	}
	...
	rcfg := &testing.RuntimeConfig{
		...
		// TODO(crbug.com/1035940): Support OutDir.
		// TODO(crbug.com/1035940): Support Vars.
...
func (st *fixtureStack) Reset(ctx context.Context) error {
	...
	case statusRed:
		return nil
	case statusYellow:
		return errors.New("BUG: Reset called for an yellow fixture stack")
	}
...
type statefulFixture struct {
	...
	setUp    bool        // true if the fixture is green or yellow
...
func (f *statefulFixture) RunTearDown(ctx context.Context) error {
	...
	// TODO(crbug.com/1035940): Support timing log.
