...
// resolveExternalDataLinks downloads missing or outdated external data files associated with tests.
// This function does not return errors; instead it tries to download files as far as possible and
// logs encountered errors with lf.
func resolveExternalDataLinks(dataDir string, tests []*testing.Test, cl storage.Client, lf func(msg string)) {
...
// prepareDownloads computes which external data files have to be downloaded.
// It also removes outdated files so that they are never used even in case of download errors.
func prepareDownloads(dataDir string, tests []*testing.Test, lf func(msg string)) []*downloadJob {
	...
	for _, t := range tests {
		for _, name := range t.Data {
			...
			outdated := os.IsNotExist(err) || linkStat.ModTime().After(destStat.ModTime())
			if outdated {
				// Remove outdated files early so that they are never used.
				os.Remove(destPath)
			}
			...
			// To check consistency, create an entry in urlToJob even if the file is outdated.
	...
	if hasErr {
		lf("Encountered some errors on scanning external data link files; corresponding tests will fail")
	}
	return jobs
}

// loadExternalLink loads a JSON file for ExternalLink.
...
func runDownloads(dataDir string, jobs []*downloadJob, cl storage.Client, lf func(msg string)) {
	...
	for range jobs {
		...
		if res.err != nil {
			...
			lf(fmt.Sprintf("Downloaded %s (%d bytes, %.0fs, %.2fMB/s)", res.url, res.size, secs, mbs))
		}
	}
	if hasErr {
		lf("Failed to download some external data files; corresponding tests will fail")
	}
}

// runDownload downloads an external data file.
func runDownload(dataDir string, job *downloadJob, cl storage.Client) (size int64, err error) {
	...
	size, err = cl.Download(context.TODO(), f, job.link.URL)
	if err != nil {
		return 0, err
	}
