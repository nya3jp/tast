...
func TestResultsHandlerSeenHeartbeat(t *gotesting.T) {
	...
	h, err := newResultsHandler(context.Background(), cfg, nil, nil)
	if err != nil {
	...
	if _, err := h.processMessages(mch, nil); err != nil {
...
func TestWritePartialResults(t *gotesting.T) {
	...
	expRes := []TestResult{
		...
		TestResult{
			...
			Start: test2Start,
			Errors: []TestError{
				TestError{Error: testing.Error{Reason: test2Reason}},
				TestError{Error: testing.Error{Reason: incompleteTestMsg}},
			},
			OutDir: filepath.Join(cfg.ResDir, testLogsDir, test2Name),
		},
	}
...
func TestUnfinishedTest(t *gotesting.T) {
	...
	emptyDiag := func() string { return "" }
	goodDiag := func() string { return diagMsg }
