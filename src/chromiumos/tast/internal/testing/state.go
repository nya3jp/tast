...
// an entity metadata (e.g. entity.ContextServiceDeps), or emit logs for an
...
type EntityRoot struct {
	ce  *entity.CurrentEntity // current entity info to be available via context.Context
...
func NewEntityRoot(ce *entity.CurrentEntity, cfg *RuntimeConfig, out OutputStream) *EntityRoot {
	return &EntityRoot{
		ce:  ce,
		cfg: cfg,
		out: out,
...
func NewContext(ctx context.Context, ec *entity.CurrentEntity, log func(msg string)) context.Context {
	ctx = logging.NewContext(ctx, log)
	ctx = entity.WithCurrentEntity(ctx, ec)
	return ctx
}
