...
func safeCall(ctx context.Context, name string, timeout, gracePeriod time.Duration, ph panicHandler, f func(ctx context.Context)) error {
	...
	// The main goroutine attempts to a token when it sees timeout or
	// context cancellation. If it successfully takes a token, safeCall
	...
	// ph (if f panicked). Until the goroutine finishes safeCall will not
	// return.
	...
	go func() {
		defer func() {
			...
			if val != nil {
				// We must call ph on this goroutine to include the panic location
				// in the stack trace.
				ph(val)
			}
		}()
