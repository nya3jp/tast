...
// sshConfigBlockType represents type of SSH config block.
...
// sshConfig is a hierarchy representation of a SSH configuration.
...
// ResolveHostFromFiles takes an address, base directrory (affects default include path) and a list of SSH configuration files and returns the resolved hostname.
func ResolveHostFromFiles(addr string, configFiles []string) (resolvedHost string, err error) {
	...
	// Include files without absolute paths are assumed to be in ~/.ssh if it is included
	// from a user configuration file or /etc/ssh if included from the system configuration file.
	sysConfigBaseDir := "/etc/ssh"
	var userConfigBaseDir string
	if homeDir, err := os.UserHomeDir(); err == nil {
		userConfigBaseDir = filepath.Join(homeDir, ".ssh")
	}
	sc := sshConfig{
	...
	for _, f := range configFiles {
		baseDir := userConfigBaseDir
		if f == "/etc/ssh/ssh_config" {
			baseDir = sysConfigBaseDir
		}
		if err = readFiles(f, baseDir, openedFiles, &sc); err != nil {
			return "", err
		}
	}
...
// The argument baseDir is used for Include statement withour absolute path.
func readFiles(configFileName, baseDir string, openedFiles map[string]struct{}, sc *sshConfig) error {
	fileNames, err := resolvePathToPaths(configFileName, baseDir)
	if err != nil {
		return err
	}
...
// The argument baseDir is used for Include statement withour absolute path.
func readFile(configFileName, baseDir string, openedFiles map[string]struct{}, parentConfig *sshConfig) error {
	...
	for scanner.Scan() {
		...
		case strings.EqualFold(strs[0], "Include"):
			for _, f := range strs[1:] {
				if err = readFiles(f, baseDir, openedFiles, curConfig); err != nil {
					return err
				}
			}
...
func resolvePathToPaths(f, baseDir string) ([]string, error) {
	...
	return filepath.Glob(filepath.Clean(f))
}
